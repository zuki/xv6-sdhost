...
- sys_pipe2()でユーザ領域のポインタに直接fdをセットしていたがセットされなかった。一旦、カーネル変数で受けて、ユーザ領域にmemmoveするようにした。これによりpipeによるfdの調整は正常にできるようになった。

- `/bin/grep abc test.txt | /bin/wc`を実行すると`kern/sdhost.c:1330: assertion failed.`が発生する
    - 一旦、`/bin/grep abc test.txt`を実行後に改めて`grep | wc`を実行すると発生しない
    - grepでtest.txtの読み込みが終わらないうちにwcが実行されるからか。一度、grepを実行
    していているとキャッシュに残っているので読み込みをする必要がないのでエラーに
    ならないのではないか
    - 本来、この手のコマンド連鎖はプロセスグループとして処理する必要があるはずなので
    今後の課題とする
...
kern/sdhost.c:1330: assertion failed.
kern/drivers/console.c:264: kernel panic at cpu 1.
```

## 12月29日

- pipe.cでlockをspinlockに変更
- rfileにO_RDONLY、wfileにO_wRONLYフラグを各々つけ、pipe_close()でfileを判断
- vfs_create_pipe()でpipe->wrteopen=1とする
...
abcdef
$ /bin/grep abc test.txt | /bin/wc
[3]pipe_close: mode: 0x401      // wfile
1 1 7                           // 正しい
[2]pipe_close: mode: 0x0        // rfile
```

- binディレクトリで`echo abc > test2.txt`でエラー

```bash
$ cd /bin
$ echo abc > test2.txt
[3]sys_openat: vfs_open test2.txt error: -12    // ENOMEM
open test2.txt failed
[3]exit: exit: pid 14, err 1
```

- /binはエントリが15で1ブロックをちょうど使い果たしていた。
- v6_dirlink()でディレクトリサイズ内に空きエントリがない場合、
  エラーにしていた。この場合はディレクトリサイズを拡張して
  エントリを追加するべきでそのように修正した（実は、vfs移行時の
  修正でdirlink()に無用な修正を追加していた）。
...
- 何もしないと以下のエラーが発生する。カーネルスレッドの処理の問題だと
  思われるが、通常、endpoint->nextpidには`0, 1, 2`しか設定されないので
  この値はありえない。endpoint構造体全体がおかしな値で上書きされている
  と思うが、とりあえずassert(0)を外して様子を見ることにする

```bash
$ [0]usb_endpoint_skip_pid: bad next pid: 1431655765
kern/usb/usb_endpoint.c:153: assertion failed.
kern/drivers/console.c:264: kernel panic at cpu 0.
```
