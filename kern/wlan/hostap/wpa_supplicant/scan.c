                ...
                bool default_ies, bool next)
{
    ...
    if (default_ies) {
        if (params->extra_ies_len) {
            os_free((uint8_t *) params->extra_ies);
    ...
    struct wpa_supplicant *wpa_s, struct wpa_driver_scan_params *params)
{
    ...
    if (params->freqs == NULL && wpa_s->p2p_in_invitation) {
        ...
        if (wpa_s->p2p_in_invitation == 1 && ssid) {
            ...
            const uint8_t *bssid = ssid->bssid_set ? ssid->bssid : NULL;
...
static void wpas_add_interworking_elements(struct wpa_supplicant *wpa_s,
                       struct wpabuf *buf)
{
    wpabuf_put_uint8_t(buf, WLAN_EID_INTERWORKING);
    wpabuf_put_uint8_t(buf, is_zero_ether_addr(wpa_s->conf->hessid) ? 1 :
              1 + ETH_ALEN);
    wpabuf_put_uint8_t(buf, wpa_s->conf->access_network_type);
    /* No Venue Info */
                        ...
                        struct wpabuf **ie)
{
    ...
    wpabuf_put_uint8_t(*ie, 3); /* FILS Request attribute length */
    wpabuf_put_uint8_t(*ie, WLAN_EID_EXT_FILS_REQ_PARAMS);
    /* Parameter control bitmap */
    wpabuf_put_uint8_t(*ie, 0);
...
static struct wpabuf * wpa_supplicant_ml_probe_ie(int mld_id, uint16_t links)
{
    struct wpabuf *extra_ie;
    uint16_t control = MULTI_LINK_CONTROL_TYPE_PROBE_REQ;
    ...
    wpabuf_put_uint8_t(extra_ie, WLAN_EID_EXTENSION);
    len_pos = wpabuf_put(extra_ie, 1);
    wpabuf_put_uint8_t(extra_ie, WLAN_EID_EXT_MULTI_LINK);

    wpabuf_put_le16(extra_ie, control);

    /* common info length and MLD ID (if requested) */
    if (mld_id >= 0) {
        wpabuf_put_uint8_t(extra_ie, 2);
        wpabuf_put_uint8_t(extra_ie, mld_id);

        wpa_printf(MSG_DEBUG, "MLD: ML probe targeted at MLD ID %d",
               mld_id);
    } else {
        wpabuf_put_uint8_t(extra_ie, 1);
    ...
    for_each_link(links, link_id) {
        wpabuf_put_uint8_t(extra_ie, EHT_ML_SUB_ELEM_PER_STA_PROFILE);
    ...
    *len_pos = (uint8_t *) wpabuf_put(extra_ie, 0) - len_pos - 1;
...
static void wpa_add_scan_ssid(struct wpa_supplicant *wpa_s,
                  struct wpa_driver_scan_params *params,
                  size_t max_ssids, const uint8_t *ssid, size_t ssid_len)
{
    unsigned int j;
...
const uint8_t * wpa_scan_get_ml_ie(const struct wpa_scan_res *res, uint8_t type)
{
    size_t ie_len = res->ie_len;
    ...
    return get_ml_ie((const uint8_t *) (res + 1), ie_len, type);
}
 ...
 */
static int wpas_channel_width_tx_pwr(const uint8_t *ies, size_t ies_len,
                     enum chan_width cw)
{
 ...
 */
int wpas_channel_width_rssi_bump(const uint8_t *ies, size_t ies_len,
                 enum chan_width cw)
{
...
int wpas_adjust_snr_by_chanwidth(const uint8_t *ies, size_t ies_len,
...
static int wpa_scan_result_compar(const void *a, const void *b)
{
    ...
    if (wa->flags & wb->flags & WPA_SCAN_LEVEL_DBM) {
        ...
        snr_a_full = wpas_adjust_snr_by_chanwidth((const uint8_t *) (wa + 1),
                              ies_len, wa->max_cw,
                              wa->snr);
        snr_a = MIN(snr_a_full, GREAT_SNR);
        ies_len = wb->ie_len ? wb->ie_len : wb->beacon_ie_len;
        snr_b_full = wpas_adjust_snr_by_chanwidth((const uint8_t *) (wb + 1),
                              ies_len, wb->max_cw,
                              wb->snr);
...
static void dump_scan_res(struct wpa_scan_results *scan_res)
{
    ...
    for (i = 0; i < scan_res->num; i++) {
        ...
        uint8_t *pos;
        const uint8_t *ssid_ie, *ssid = NULL;
        size_t ssid_len = 0;
...
unsigned int wpas_get_est_tpt(const struct wpa_supplicant *wpa_s,
                  const uint8_t *ies, size_t ies_len, int rate,
                  int snr, int freq, enum chan_width *max_cw)
{
    ...
    if (ie && ie[1] >= 3) {
        uint8_t cw = ie[2] & VHT_OPMODE_CHANNEL_WIDTH_MASK;
        uint8_t seg0 = ie[3];
        uint8_t seg1 = ie[4];
             ...
             struct wpa_scan_res *res)
{
    ...
    int snr = res->snr;
    const uint8_t *ies = (const void *) (res + 1);
    size_t ie_len = res->ie_len;
...
/**
 * wpa_supplicant_get_scan_results - スキャン結果を取得する
 * @wpa_s: Pointer to wpa_supplicant data
...
void wpa_scan_free_params(struct wpa_driver_scan_params *params)
{
    ...
    for (i = 0; i < params->num_ssids; i++)
        os_free((uint8_t *) params->ssids[i].ssid);
    os_free((uint8_t *) params->extra_ies);
    os_free(params->freqs);
    os_free(params->filter_ssids);
     ...
     */
    os_free((uint8_t *) params->mac_addr);

    os_free((uint8_t *) params->bssid);
...
int wpas_mac_addr_rand_scan_get_mask(struct wpa_supplicant *wpa_s,
                     unsigned int type, uint8_t *mask)
{
