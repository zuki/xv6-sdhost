...
/*
 * ネットワーク構成解析用の構造体。このデータは、各ネットワークブロック変数に対する
 * 汎用パーサーを実装するために使用される。構成変数のテーブルはこのファイルの後半で
 * 定義されている（ssid_fields[]）。
 */
struct parse_data {
    /* 構成変数名 */
    ...
    /* この変数のパーサー関数。パーサー関数は、成功した場合は 0 または 1 を返す。
     * 値 0 はパラメータの値が変更された可能性があることを示し、値 1 は値が変更
     * されなかったことを意味する。エラーの場合（文字列の解析に失敗した場合）は
     * -1 を返すことで示される */
    ...
    /* 0 = この変数はデバッグ出力およびctrl_ifaceに含めることができる
     * 1 = この変数には鍵情報や機密データが含まれており、明示的に要求されない限り、
     *     デバッグ出力に含めてはならない。また、この変数は ctrl_iface を通じて
     *     読み取ることはできない。
     */
    int key_data;
};
                ...
                int line, const char *value)
{
    ...
    // キーワードはデバッグ出力しないようにする: wpa_debug_show_keys = 0で出力しない
    if (data->key_data) {
...
set:
    dst = (char **) (((uint8_t *) ssid) + (long) data->param1);
    dst_len = (size_t *) (((uint8_t *) ssid) + (long) data->param2);
...
#ifndef NO_CONFIG_WRITE
static char * wpa_config_write_string_ascii(const uint8_t *value, size_t len)
{
    char *buf;
...
static char * wpa_config_write_string_hex(const uint8_t *value, size_t len)
{
    char *buf;
...
static char * wpa_config_write_string(const uint8_t *value, size_t len)
{
    if (value == NULL)
        return NULL;
                   ...
                   struct wpa_ssid *ssid)
{
    ...
    src = (char **) (((uint8_t *) ssid) + (long) data->param1);
    if (*src == NULL)
        return NULL;

    if (data->param2)
        len = *((size_t *) (((uint8_t *) ssid) + (long) data->param2));
    else
        len = os_strlen(*src);

    return wpa_config_write_string((const uint8_t *) *src, len);
}
#endif /* NO_CONFIG_WRITE */
                     ...
                     bool check_range)
{
    ...
    dst = (int *) (((uint8_t *) ssid) + (long) data->param1);
                   ...
                   struct wpa_ssid *ssid)
{
    ...
    src = (int *) (((uint8_t *) ssid) + (long) data->param1);
...
static int wpa_config_parse_addr_list(const struct parse_data *data,
                      int line, const char *value,
                      uint8_t **list, size_t *num, char *name,
                      uint8_t abort_on_error, uint8_t masked)
{
    const char *pos;
    uint8_t *buf, *n, addr[2 * ETH_ALEN];
    size_t count;
...
static char * wpa_config_write_addr_list(const struct parse_data *data,
                     const uint8_t *list, size_t num, char *name)
{
    char *value, *end, *pos;
    int res;
    size_t i;
    ...
    for (i = num; i > 0; i--) {
        const uint8_t *a = list + (i - 1) * 2 * ETH_ALEN;
        const uint8_t *m = a + ETH_ALEN;
                ...
                const char *value)
{
    ...
    if (*value == '"') {
        ...
        wpa_hexdump_ascii_key(MSG_MSGDUMP, "PSK (ASCII passphrase)",
                      (uint8_t *) value, len);
        if (has_ctrl_char((uint8_t *) value, len)) {
            wpa_printf(MSG_ERROR,
                ...
                const char *value)
{
    ...
    wpa_hexdump(MSG_MSGDUMP, "eap methods",
            (uint8_t *) methods, num_methods * sizeof(*methods));
                     ...
                     const char *value)
{
    ...
    if (os_strncmp(value, "ext:", 4) == 0) {
        ...
        if (!name)
            return -1;
        bin_clear_free(ssid->eap.password, ssid->eap.password_len);
        ssid->eap.password = (uint8_t *) name;
                         ...
                         const char *value)
{
    ...
    if (os_strncmp(value, "ext:", 4) == 0) {
        ...
        bin_clear_free(ssid->eap.machine_password,
                   ssid->eap.machine_password_len);
        ssid->eap.machine_password = (uint8_t *) name;
    ...
    if (os_strncmp(value, "hash:", 5) != 0) {
        ...
        bin_clear_free(ssid->eap.machine_password,
                   ssid->eap.machine_password_len);
        ssid->eap.machine_password = (uint8_t *) tmp;
...
static int wpa_config_parse_wep_key(uint8_t *key, size_t *len, int line,
...
/**
 * wpa_config_add_prio_network - ネットワークを優先度リストに追加する
 * @config:  wpa_config_read()による構成データ
 * @ssid: リストに追加するネットワーク構成データ構造体へのポインタ
 * Returns: 成功の場合は 0, 失敗の場合は -1
 *
 * この関数はネットワークブロックをネットワークの優先順位リストに追加するために
 * 使用される。すべての構成を読み込む際には、ネットワークごとにこの関数を呼び出す
 * 必要がある。また、wpa_config_update_prio_list() を呼び出して優先順位を
 * 更新する際にも、間接的にこの関数を使用することができる。
 */
                ...
                struct wpa_ssid *ssid)
{
    ...
    /*
     * このネットワークに対して設定された優先度レベルに対応する
     * 優先度リストが存在する場合は、そのリストに追加する。
     */
    ...
    /* この優先度を持つ最初のネットワーク - 新規優先度リストを追加する */
...
/**
 * wpa_config_set - ネットワーク構成ファイルの変数をセットする
 * @ssid: ネットワーク構成データへのポインタ
 * @var: 変数名, "ssid" など
 * @value: 変数の値
 * @line: 構成ファイル上の行番号、使用しない場合は 0
 * Returns: 成功時で値が変更された場合は0、以前に設定された値に変更が
 * ない場合は1、失敗時は-1を返す
 *
 * この関数を使用すると、構成ファイルと管理インタフェースからの入力の双方に
 * 基づいて、ネットワーク構成変数を設定することができる。value パラメータは
 * テキスト形式の構成ファイルで使用されているものと同じ形式で指定する必要がある。
 * たとえば、文字列には二重引用符を使用する。
 */
...
/**
 * wpa_config_update_psk - パスフレーズまたはSSIDに基づいてWPA PSKを更新する
 * @ssid: ネットワーク構成データ構造体へのポインタ
 *
 * ネットワーク構成において、SSIDまたはパスフレーズが変更された場合は、
 * WPA PSKを更新するためにこの関数を呼び出す必要がある。
 */
                          ...
                          const char *value)
{
    ...
    proto = os_realloc_array(cred->req_conn_capab_proto,
                 cred->num_req_conn_capab + 1, sizeof(uint8_t));
    if (proto == NULL)
        return -1;
...
static int
wpa_config_set_cred_ois(uint8_t cred_ois[MAX_ROAMING_CONS][MAX_ROAMING_CONS_OI_LEN],
            size_t cred_ois_len[MAX_ROAMING_CONS],
            unsigned int *cred_num_ois,
            const char *value)
{
    uint8_t ois[MAX_ROAMING_CONS][MAX_ROAMING_CONS_OI_LEN];
...
/**
 * wpa_config_debug_dump_networks - 構成されたネットワークをデバッグダンプ
 *
 * @config: Configuration data from wpa_config_read()
 */
                 ...
                 const char *pos, bool check_range)
{
    ...
    dst = (int *) (((uint8_t *) config) + (long) data->param1);
                       ...
                       const char *pos)
{
    ...
    dst = (char **) (((uint8_t *) config) + (long) data->param1);
    if (*dst)
        prev_len = os_strlen(*dst);
    else
        prev_len = 0;
                       ...
                       const char *pos)
{
    ...
    dst = (struct wpabuf **) (((uint8_t *) config) + (long) data->param1);
...
static int wpa_config_get_int(const char *name, struct wpa_config *config,
                  ...
                  int pretty_print)
{
    int *val = (int *) (((uint8_t *) config) + (long) offset);
...
static int wpa_config_get_str(const char *name, struct wpa_config *config,
                  ...
                  int pretty_print)
{
    char **val = (char **) (((uint8_t *) config) + (long) offset);
    int res;
...
static int wpa_config_get_ipv4(const char *name, struct wpa_config *config,
                   ...
                   int pretty_print)
{
    void *val = ((uint8_t *) config) + (long) offset;
    int res;
 ...
 * Returns: 成功時で、値が変更された場合は 0、すでにセットされていた変数値を
 * 変更しなかった場合は 1, 失敗時は -1
 *
 * この関数を使用すると、構成ファイルと管理インタフェースからの入力の双方に
 * 基づいて、グローバル構成変数を設定することができる。value パラメータは
 * テキスト形式の構成ファイルで使用されているのと同じ形式で指定する必要がある。
 * たとえば、文字列には二重引用符を使用する。
 */
