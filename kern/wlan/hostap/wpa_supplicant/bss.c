...
static void wpa_bss_set_hessid(struct wpa_bss *bss)
{
	...
	const uint8_t *ie = wpa_bss_get_ie(bss, WLAN_EID_INTERWORKING);
...
static int are_ies_equal(const struct wpa_bss *old,
			 const struct wpa_scan_res *new_res, uint32_t ie)
{
	...
	if (is_multi) {
		...
		old_ie = old_ie_buff ? wpabuf_head_uint8_t(old_ie_buff) : NULL;
		new_ie = new_ie_buff ? wpabuf_head_uint8_t(new_ie_buff) : NULL;
...
static uint32_t wpa_bss_compare_res(const struct wpa_bss *old,
...
void notify_bss_changes(struct wpa_supplicant *wpa_s, uint32_t changes,
			...
			int new_scan)
{
	...
	wpa_printf(MSG_DEBUG, "BSS: last_scan_res_used=%llu/%llu",
 ...
 */
const uint8_t * wpa_bss_get_ie_beacon(const struct wpa_bss *bss, uint8_t ie)
{
	const uint8_t *ies;
 ...
 * entry.
 */
const uint8_t * wpa_bss_get_ie_ext(const struct wpa_bss *bss, uint8_t ext)
{
...
#ifdef CONFIG_FILS
const uint8_t * wpa_bss_get_fils_cache_id(const struct wpa_bss *bss)
{
	const uint8_t *ie;
...
wpa_bss_parse_ml_rnr_ap_info(struct wpa_supplicant *wpa_s,
			     struct wpa_bss *bss, uint8_t mbssid_idx,
			     const struct ieee80211_neighbor_ap_info *ap_info,
			     size_t len, uint16_t *seen, uint16_t *missing,
			     struct wpa_ssid *ssid)
{
	const uint8_t *pos, *end;
	const uint8_t *mld_params;
	uint8_t count, mld_params_offset;
	uint8_t i, type, link_id;
				   ...
				   uint8_t *ap_mld_id)
{
	...
	uint16_t missing = 0;
	uint16_t seen;
	const uint8_t *ies_pos = wpa_bss_ie_ptr(bss);
 ...
 */
uint16_t wpa_bss_parse_reconf_ml_element(struct wpa_supplicant *wpa_s,
				    struct wpa_bss *bss)
{
	struct ieee802_11_elems elems;
	struct wpabuf *mlbuf;
	const uint8_t *pos = wpa_bss_ie_ptr(bss);
	size_t len = bss->ie_len ? bss->ie_len : bss->beacon_ie_len;
	const struct ieee80211_eht_ml *ml;
	uint16_t removed_links = 0;
	uint8_t ml_common_len;
	...
	while (len >= 2 + sizeof(struct ieee80211_eht_per_sta_profile)) {
		...
		if  (*pos == EHT_ML_SUB_ELEM_PER_STA_PROFILE) {
			const struct ieee80211_eht_per_sta_profile *sta_prof =
				...
				(pos + 2);
			uint16_t control = le_to_host16(sta_prof->sta_control);
			uint8_t link_id;
