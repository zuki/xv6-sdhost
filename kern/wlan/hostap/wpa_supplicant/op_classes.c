...
static int get_center_80mhz(struct hostapd_hw_modes *mode, uint8_t channel,
			    const uint8_t *center_channels, size_t num_chan)
{
	size_t i;
				      ...
				      uint8_t op_class, uint8_t channel)
{
	...
	const uint8_t *center_channels;
	size_t num_chan;
	const uint8_t center_channels_5ghz[] = { 42, 58, 106, 122, 138, 155, 171 };
	const uint8_t center_channels_6ghz[] = { 7, 23, 39, 55, 71, 87, 103, 119,
	...
	for (i = 0; i < 4; i++) {
		unsigned int flags;
		uint8_t adj_chan = center_chan - 6 + i * 4;
...
static int get_center_160mhz(struct hostapd_hw_modes *mode, uint8_t channel,
			     const uint8_t *center_channels, size_t num_chan)
{
	unsigned int i;
				       ...
				       uint8_t op_class, uint8_t channel)
{
	...
	const uint8_t *center_channels;
	size_t num_chan;
	const uint8_t center_channels_5ghz[] = { 50, 114, 163 };
	const uint8_t center_channels_6ghz[] = { 15, 47, 79, 111, 143, 175, 207 };
	...
	for (i = 0; i < 8; i++) {
		unsigned int flags;
		uint8_t adj_chan = center_chan - 14 + i * 4;
...
static int get_center_320mhz(struct hostapd_hw_modes *mode, uint8_t channel,
			     const uint8_t *center_channels, size_t num_chan)
{
	unsigned int i;
				       ...
				       uint8_t op_class, uint8_t channel)
{
	...
	const uint8_t *center_channels;
	size_t num_chan;
	const uint8_t center_channels_6ghz[] = { 31, 63, 95, 127, 159, 191 };
	...
	for (i = 0; i < 16; i++) {
		unsigned int flags;
		uint8_t adj_chan = center_chan - 30 + i * 4;
				   ...
				   const struct oper_class_map *op_class)
{
	...
	if (op_class->op_class == 128) {
		uint8_t channels[] = { 42, 58, 106, 122, 138, 155, 171 };
...
static int wpas_sta_secondary_channel_offset(struct wpa_bss *bss, uint8_t *current,
					     uint8_t *channel)
{
			     ...
			     struct wpa_bss *bss, uint8_t *pos, size_t len)
{
	...
	wpabuf_put_uint8_t(buf, WLAN_EID_SUPPORTED_OPERATING_CLASSES);
	/* Will set the length later, putting a placeholder */
	ie_len = wpabuf_put(buf, 1);
	wpabuf_put_uint8_t(buf, current);

	for (op = 0; global_op_class[op].op_class; op++) {
		bool supp;
		uint8_t op_class = global_op_class[op].op_class;
		...
		wpabuf_put_uint8_t(buf, global_op_class[op].op_class);
	}
	...
	if ((op128 && op130) || (op133 && op135)) {
		...
		if (op128 && op130) {
			wpabuf_put_uint8_t(buf, 130);
			wpabuf_put_uint8_t(buf, 128);
		}
		if (op133 && op135) {
			wpabuf_put_uint8_t(buf, 135);
			wpabuf_put_uint8_t(buf, 133);
		}
	}
