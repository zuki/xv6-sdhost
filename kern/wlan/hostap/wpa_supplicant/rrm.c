				       ...
				       void *cb_ctx)
{
	...
	wpabuf_put_uint8_t(buf, WLAN_ACTION_RADIO_MEASUREMENT);
	wpabuf_put_uint8_t(buf, WLAN_RRM_NEIGHBOR_REPORT_REQUEST);
	wpabuf_put_uint8_t(buf, wpa_s->rrm.next_neighbor_rep_token);
	if (ssid) {
		wpabuf_put_uint8_t(buf, WLAN_EID_SSID);
		wpabuf_put_uint8_t(buf, ssid->ssid_len);
		wpabuf_put_data(buf, ssid->ssid, ssid->ssid_len);
	}

	if (lci) {
		/* IEEE P802.11-REVmc/D5.0 9.4.2.21 */
		wpabuf_put_uint8_t(buf, WLAN_EID_MEASURE_REQUEST);
		wpabuf_put_uint8_t(buf, MEASURE_REQUEST_LCI_LEN);

		/*
		 * Measurement token; nonzero number that is unique among the
		 * Measurement Request elements in a particular frame.
		 */
		wpabuf_put_uint8_t(buf, 1); /* Measurement Token */

		/*
		 * Parallel, Enable, Request, and Report bits are 0, Duration is
		 * reserved.
		 */
		wpabuf_put_uint8_t(buf, 0); /* Measurement Request Mode */
		wpabuf_put_uint8_t(buf, MEASURE_TYPE_LCI); /* Measurement Type */

		/* IEEE P802.11-REVmc/D5.0 9.4.2.21.10 - LCI request */
		/* Location Subject */
		wpabuf_put_uint8_t(buf, LOCATION_SUBJECT_REMOTE);
		 ...
		 */
		wpabuf_put_uint8_t(buf, LCI_REQ_SUBELEM_MAX_AGE);
		wpabuf_put_uint8_t(buf, 2);
	...
	if (civic) {
		/* IEEE P802.11-REVmc/D5.0 9.4.2.21 */
		wpabuf_put_uint8_t(buf, WLAN_EID_MEASURE_REQUEST);
		wpabuf_put_uint8_t(buf, MEASURE_REQUEST_CIVIC_LEN);

		/*
		 * Measurement token; nonzero number that is unique among the
		 * Measurement Request elements in a particular frame.
		 */
		wpabuf_put_uint8_t(buf, 2); /* Measurement Token */
		...
		wpabuf_put_uint8_t(buf, 0); /* Measurement Request Mode */
		/* Measurement Type */
		wpabuf_put_uint8_t(buf, MEASURE_TYPE_LOCATION_CIVIC);

		/* IEEE P802.11-REVmc/D5.0 9.4.2.21.14:
		 * Location Civic request */
		/* Location Subject */
		wpabuf_put_uint8_t(buf, LOCATION_SUBJECT_REMOTE);
		wpabuf_put_uint8_t(buf, 0); /* Civic Location Type: IETF RFC 4776 */
		/* Location Service Interval Units: Seconds */
		wpabuf_put_uint8_t(buf, 0);
...
static int wpas_rrm_report_elem(struct wpabuf **buf, uint8_t token, uint8_t mode, uint8_t type,
				const uint8_t *data, size_t data_len)
{
	if (wpabuf_resize(buf, 5 + data_len))
		return -1;

	wpabuf_put_uint8_t(*buf, WLAN_EID_MEASURE_REPORT);
	wpabuf_put_uint8_t(*buf, 3 + data_len);
	wpabuf_put_uint8_t(*buf, token);
	wpabuf_put_uint8_t(*buf, mode);
			  ...
			  struct wpabuf **buf)
{
	...
	const uint8_t *subelem;
	const uint8_t *request = req->variable;
					  ...
					  const uint8_t *data, size_t len)
{
	...
	wpabuf_put_uint8_t(report, WLAN_ACTION_RADIO_MEASUREMENT);
	wpabuf_put_uint8_t(report, WLAN_RRM_RADIO_MEASUREMENT_REPORT);
	wpabuf_put_uint8_t(report, wpa_s->rrm.token);
...
static int wpas_rrm_beacon_rep_update_last_frame(uint8_t *pos, size_t len)
{
				     ...
				     struct wpabuf *buf)
{
	...
	uint8_t *pos = wpabuf_mhead_uint8_t(buf), *next = pos;
...
static int wpas_add_channel(uint8_t op_class, uint8_t chan, uint8_t num_primary_channels,
			    int *freqs)
{
	size_t i;

	for (i = 0; i < num_primary_channels; i++) {
		uint8_t primary_chan = chan - (2 * num_primary_channels - 2) + i * 4;
...
static int * wpas_add_channels(const struct oper_class_map *op,
			       ...
			       const uint8_t *channels, const uint8_t size)
{
	int *freqs, *next_freq;
	uint8_t num_primary_channels, i;
	uint8_t num_chans;
	...
	for  (i = 0; i < num_chans; i++) {
		uint8_t chan = channels ? channels[i] : op->min_chan + i * op->inc;
...
static int * wpas_op_class_freqs(const struct oper_class_map *op,
				 struct hostapd_hw_modes *mode)
{
	uint8_t channels_80mhz_5ghz[] = { 42, 58, 106, 122, 138, 155, 171 };
	uint8_t channels_160mhz_5ghz[] = { 50, 114, 163 };
	uint8_t channels_80mhz_6ghz[] = { 7, 23, 39, 55, 71, 87, 103, 119, 135, 151,
				     167, 183, 199, 215 };
	uint8_t channels_160mhz_6ghz[] = { 15, 47, 79, 111, 143, 175, 207 };
	uint8_t channels_320mhz_6ghz[] = { 31, 63, 95, 127, 159, 191 };
...
static int * wpas_channel_report_freqs(struct wpa_supplicant *wpa_s,
				       const char *country, const uint8_t *subelems,
				       size_t len)
{
	int *freqs = NULL, *new_freqs;
	const uint8_t *end = subelems + len;

	while (end - subelems > 2) {
		const struct oper_class_map *op;
		const uint8_t *ap_chan_elem, *pos;
		uint8_t left;
		struct hostapd_hw_modes *mode;
...
static int * wpas_beacon_request_freqs(struct wpa_supplicant *wpa_s,
				       ...
				       const uint8_t *subelems, size_t len)
{
...
int wpas_get_op_chan_phy(int freq, const uint8_t *ies, size_t ies_len,
			 uint8_t *op_class, uint8_t *chan, uint8_t *phy_type)
{
	const uint8_t *ie;
...
static int wpas_beacon_rep_add_frame_body(struct bitfield *eids,
					  ...
					  struct wpa_bss *bss, uint8_t *buf,
					  size_t buf_len, const uint8_t **ies_buf,
					  size_t *ie_len, int add_fixed)
{
	const uint8_t *ies = *ies_buf;
...
static int wpas_add_beacon_rep_elem(struct beacon_rep_data *data,
				    ...
				    const uint8_t **ie, size_t *ie_len, uint8_t idx)
{
	int ret;
	uint8_t *buf, *pos;
...
static int wpas_add_beacon_rep(struct wpa_supplicant *wpa_s,
			       ...
			       uint64_t start, uint64_t parent_tsf)
{
	struct beacon_rep_data *data = &wpa_s->beacon_rep_data;
	const uint8_t *ies = wpa_bss_ie_ptr(bss);
...
static void wpas_rrm_scan_timeout(void *eloop_ctx, void *timeout_ctx)
{
	...
	uint16_t prev_duration = params->duration;
...
static int wpas_rm_handle_beacon_req_subelem(struct wpa_supplicant *wpa_s,
					     ...
					     uint8_t sid, uint8_t slen, const uint8_t *subelem)
{
	struct bitfield *eids;
	uint8_t report_info, i;
...
wpas_rm_handle_beacon_req(struct wpa_supplicant *wpa_s,
			  uint8_t elem_token, int duration_mandatory,
			  ...
			  size_t len, struct wpabuf **buf)
{
	...
	if (os_get_random((uint8_t *) &_rand, sizeof(_rand)) < 0)
		_rand = os_random();
...
wpas_rrm_process_msr_req_elems(struct wpa_supplicant *wpa_s, const uint8_t *pos,
			       size_t len)
{
	struct wpabuf *buf = NULL;
					      ...
					      int rssi)
{
	...
	wpabuf_put_uint8_t(buf, WLAN_ACTION_RADIO_MEASUREMENT);
	wpabuf_put_uint8_t(buf, WLAN_RRM_LINK_MEASUREMENT_REPORT);
