...
#if defined(IEEE8021X_EAPOL) || !defined(CONFIG_NO_WPA)
static uint8_t * wpa_alloc_eapol(const struct wpa_supplicant *wpa_s, uint8_t type,
 ...
 */
static int wpa_supplicant_eapol_send(void *ctx, int type, const uint8_t *buf,
				     size_t len)
{
	struct wpa_supplicant *wpa_s = ctx;
	uint8_t *msg, *dst, bssid[ETH_ALEN];
	size_t msglen;
	int res;
...
static uint8_t * _wpa_alloc_eapol(void *wpa_s, uint8_t type,
			     const void *data, uint16_t data_len,
			     size_t *msg_len, void **data_pos)
{
...
static int _wpa_ether_send(void *wpa_s, const uint8_t *dest, uint16_t proto,
...
static void _wpa_supplicant_deauthenticate(void *wpa_s, uint16_t reason_code)
{
...
static int wpa_supplicant_get_bssid(void *ctx, uint8_t *bssid)
{
	struct wpa_supplicant *wpa_s = ctx;
...
static int wpa_supplicant_mlme_setprotection(void *wpa_s, const uint8_t *addr,
					     int protection_type,
					     int key_type)
{
...
static int wpa_supplicant_add_pmkid(void *_wpa_s, void *network_ctx,
				    ...
				    uint32_t pmk_lifetime, uint8_t pmk_reauth_threshold,
				    int akmp)
{
	struct wpa_supplicant *wpa_s = _wpa_s;
	struct wpa_ssid *ssid;
...
#ifdef CONFIG_IEEE80211R
static int wpa_supplicant_update_ft_ies(void *ctx, const uint8_t *md,
...
static int wpa_supplicant_send_ft_action(void *ctx, uint8_t action,
...
static int wpa_supplicant_mark_authenticated(void *ctx, const uint8_t *target_ap)
{
	struct wpa_supplicant *wpa_s = ctx;
...
static int wpa_supplicant_send_tdls_mgmt(void *ctx, const uint8_t *dst,
...
static int wpa_supplicant_tdls_oper(void *ctx, int oper, const uint8_t *peer)
{
	struct wpa_supplicant *wpa_s = ctx;
...
static int wpa_supplicant_tdls_peer_addset(
	void *ctx, const uint8_t *peer, int add, uint16_t aid, uint16_t capability,
	...
	uint8_t qosinfo, int wmm, const uint8_t *ext_capab, size_t ext_capab_len,
	const uint8_t *supp_channels, size_t supp_channels_len,
	const uint8_t *supp_oper_classes, size_t supp_oper_classes_len,
...
static int wpa_supplicant_tdls_disable_channel_switch(void *ctx, const uint8_t *addr)
{
	struct wpa_supplicant *wpa_s = ctx;
...
static void wpa_supplicant_set_anon_id(void *ctx, const uint8_t *id, size_t len)
{
	struct wpa_supplicant *wpa_s = ctx;
	char *str;
	int res;
...
static int wpa_supplicant_key_mgmt_set_pmk(void *ctx, const uint8_t *pmk,
					   size_t pmk_len)
{
	struct wpa_supplicant *wpa_s = ctx;
...
static void wpa_supplicant_fils_hlp_rx(void *ctx, const uint8_t *dst, const uint8_t *src,
...
static void wpa_supplicant_transition_disable(void *_wpa_s, uint8_t bitmap)
{
	struct wpa_supplicant *wpa_s = _wpa_s;
...
static void wpa_supplicant_store_ptk(void *ctx, const uint8_t *addr, int cipher,
...
#ifdef CONFIG_PASN
static int wpa_supplicant_set_ltf_keyseed(void *_wpa_s, const uint8_t *own_addr,
