...
static void wpa_supplicant_timeout(void *eloop_ctx, void *timeout_ctx)
{
    ...
    const uint8_t *bssid = wpa_s->bssid;
...
/**
 * wpa_supplicant_initiate_eapol - EAPOLステートマシンを構成する
 * @wpa_s: Pointer to wpa_supplicant data
 *
 * 選択された認証モードに基づいてEAPOLステートマシンを構成する.
 */
                  ...
                  bool skip_default_rsne)
{
    ...
    const uint8_t *bss_wpa, *bss_rsn, *bss_rsnx, *bss_osen;
    bool wmm;
...
static void wpas_ext_capab_byte(struct wpa_supplicant *wpa_s, uint8_t *pos, int idx,
                struct wpa_bss *bss)
{
#ifndef CONFIG_NO_ROBUST_AV
...
int wpas_build_ext_capab(struct wpa_supplicant *wpa_s, uint8_t *buf,
            ...
            bool force)
{
    ...
    ssid->pt = sae_derive_pt(groups, ssid->ssid, ssid->ssid_len,
                 (const uint8_t *) password, os_strlen(password),
...
#ifdef CONFIG_FILS
static size_t wpas_add_fils_hlp_req(struct wpa_supplicant *wpa_s, uint8_t *ie_buf,
                    size_t ie_buf_len)
{
    struct fils_hlp_req *req;
                  ...
                  size_t max_wpa_ie_len)
{
    ...
    wpabuf_put_uint8_t(wfa_ie, WLAN_EID_VENDOR_SPECIFIC);
    wpabuf_put_uint8_t(wfa_ie, buf_len - 2);
    wpabuf_put_be24(wfa_ie, OUI_WFA);
    wpabuf_put_uint8_t(wfa_ie, WFA_CAPA_OUI_TYPE);
    wpabuf_put_uint8_t(wfa_ie, sizeof(wfa_capa));
...
static uint8_t * wpas_populate_assoc_ies(
    struct wpa_supplicant *wpa_s,
    struct wpa_bss *bss, struct wpa_ssid *ssid,
    ...
    enum wpa_drv_update_connect_params_mask *mask)
{
...
#ifdef CONFIG_FILS
    const uint8_t *realm, *username, *rrk;
    ...
    if (bss && (wpa_bss_get_vendor_ie(bss, WPA_IE_VENDOR_TYPE) ||
        ...
        const uint8_t *cache_id = NULL;
        const uint8_t *addr = bss->bssid;
...
pfs_fail:
    ...
    if (bss && wpa_key_mgmt_ft(wpa_sm_get_key_mgmt(wpa_s->wpa))) {
        const uint8_t *mdie = wpa_bss_get_ie(bss, WLAN_EID_MOBILITY_DOMAIN);

        if (mdie && mdie[1] >= MOBILITY_DOMAIN_ID_LEN) {
            size_t len = 0;
            const uint8_t *md = mdie + 2;
            const uint8_t *wpa_md = wpa_sm_get_ft_md(wpa_s->wpa);
...
static uint8_t wpa_ie_get_edmg_oper_chans(const uint8_t *edmg_ie)
{
...
static uint8_t wpa_ie_get_edmg_oper_chan_width(const uint8_t *edmg_ie)
{
    if (!edmg_ie || edmg_ie[1] < 6)
        return 0;
...
static void wpas_start_assoc_cb(struct wpa_radio_work *work, int deinit)
{
    ...
    if (params.mgmt_frame_protection != NO_MGMT_FRAME_PROTECTION && bss) {
        const uint8_t *rsn = wpa_bss_get_ie(bss, WLAN_EID_RSN);
        struct wpa_ie_data ie;
    ...
    params.htcaps = (uint8_t *) &htcaps;
    params.htcaps_mask = (uint8_t *) &htcaps_mask;
...
#ifdef CONFIG_OWE
static int owe_trans_ssid_match(struct wpa_supplicant *wpa_s, const uint8_t *bssid,
                const uint8_t *entry_ssid, size_t entry_ssid_len)
{
    const uint8_t *owe, *pos, *end;
    uint8_t ssid_len;
    struct wpa_bss *bss;
                 ...
                 enum frame_encryption encrypted)
{
    ...
    const uint8_t *connected_addr = wpa_s->valid_links ?
...
int wpa_supplicant_update_mac_addr(struct wpa_supplicant *wpa_s)
{
    ...
    if ((!wpa_s->p2p_mgmt ||
        ...
        const uint8_t *addr = wpa_drv_get_mac_addr(wpa_s);
        if (addr)
...
static void wpa_supplicant_rx_eapol_bridge(void *ctx, const uint8_t *src_addr,
                       const uint8_t *buf, size_t len)
{
    struct wpa_supplicant *wpa_s = ctx;
...
static const uint8_t * wpas_fst_get_bssid_cb(void *ctx)
{
    struct wpa_supplicant *wpa_s = ctx;
...
static int wpas_fst_send_action_cb(void *ctx, const uint8_t *da, struct wpabuf *data)
{
    struct wpa_supplicant *wpa_s = ctx;
...
static const struct wpabuf * wpas_fst_get_mb_ie_cb(void *ctx, const uint8_t *addr)
{
    struct wpa_supplicant *wpa_s = ctx;
...
static void wpas_fst_update_mb_ie_cb(void *ctx, const uint8_t *addr,
...
static const uint8_t * wpas_fst_get_peer_first(void *ctx,
...
static const uint8_t * wpas_fst_get_peer_next(void *ctx,
                     struct fst_get_peer_ctx **get_ctx,
                     bool mb_only)
{
    return NULL;
...
static void wpas_gas_server_tx(void *ctx, int freq, const uint8_t *da,
...
/**
 * wpa_supplicant_init - %wpa_supplicant を初期化する
 * @params: %wpa_supplicant のパラメタ
 * Returns: グローバルな %wpa_supplicant データへのポインタ, 失敗の場合は %NULL
 *
 * %wpa_supplicant の初期化に使用する. 初期化に成功すると、返された
 * データポインタを使ってネットワークインタフェースの追加・削除ができる
 * ようになる。最終的には %wpa_supplicant の廃棄にも使用できる.
 */
                          ...
                          const char *value)
{
    ...
    wpa_hexdump_ascii_key(MSG_DEBUG, "CTRL_IFACE: response value",
                  (const uint8_t *) value, os_strlen(value));

    switch (wpa_supplicant_ctrl_req_from_string(field)) {
    case WPA_CTRL_REQ_EAP_IDENTITY:
        os_free(eap->identity);
        eap->identity = (uint8_t *) os_strdup(value);
        if (!eap->identity)
            return -1;
    ...
    case WPA_CTRL_REQ_EAP_PASSWORD:
        ...
        eap->password = (uint8_t *) os_strdup(value);
        if (!eap->password)
            return -1;
    ...
    case WPA_CTRL_REQ_EAP_NEW_PASSWORD:
        ...
        eap->new_password = (uint8_t *) os_strdup(value);
        if (!eap->new_password)
            return -1;
    ...
    case WPA_CTRL_REQ_EAP_OTP:
        ...
        eap->otp = (uint8_t *) os_strdup(value);
        if (!eap->otp)
            return -1;
                ...
                const uint8_t *elem, size_t len)
{
    ...
    ie = wpabuf_mhead_uint8_t(wpa_s->vendor_elem[frame]);
...
wpa_drv_get_scan_results(struct wpa_supplicant *wpa_s, const uint8_t *bssid)
{
    struct wpa_scan_results *scan_res;
#ifdef CONFIG_TESTING_OPTIONS
    size_t idx;
...
bool wpas_ap_link_address(struct wpa_supplicant *wpa_s, const uint8_t *addr)
{
    int i;
...
int wpa_drv_send_action(struct wpa_supplicant *wpa_s, unsigned int freq,
            unsigned int wait, const uint8_t *dst, const uint8_t *src,
            const uint8_t *bssid, const uint8_t *data, size_t data_len,
            int no_cck)
{
