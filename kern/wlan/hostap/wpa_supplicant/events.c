...
static struct wpa_bss * __wpa_supplicant_get_new_bss(
    struct wpa_supplicant *wpa_s, const uint8_t *bssid, const uint8_t *ssid,
    size_t ssid_len)
{
    if (ssid && ssid_len > 0)
...
static struct wpa_bss * wpa_supplicant_get_new_bss(
    struct wpa_supplicant *wpa_s, const uint8_t *bssid)
{
    struct wpa_bss *bss = NULL;
    struct wpa_ssid *ssid = wpa_s->current_ssid;
    uint8_t drv_ssid[SSID_MAX_LEN];
    int res;
...
wpa_supplicant_update_current_bss(struct wpa_supplicant *wpa_s, const uint8_t *bssid)
{
...
static void wpa_supplicant_update_link_bss(struct wpa_supplicant *wpa_s,
                       uint8_t link_id, const uint8_t *bssid)
{
    struct wpa_bss *bss = wpa_supplicant_get_new_bss(wpa_s, bssid);
              ...
              struct wpa_bss *bss, int debug_print)
{
    ...
    const uint8_t scan_ie[2] = { WLAN_EID_SUPP_RATES, WLAN_EID_EXT_SUPP_RATES };
...
static int match_mac_mask(const uint8_t *addr_a, const uint8_t *addr_b, const uint8_t *mask)
{
    size_t i;
...
static int addr_in_list(const uint8_t *addr, const uint8_t *list, size_t num)
{
    size_t i;

    for (i = 0; i < num; i++) {
        const uint8_t *a = list + i * ETH_ALEN * 2;
...
static void owe_trans_ssid(struct wpa_supplicant *wpa_s, struct wpa_bss *bss,
               const uint8_t **ret_ssid, size_t *ret_ssid_len)
{
#ifdef CONFIG_OWE
    const uint8_t *owe, *pos, *end, *bssid;
    uint8_t ssid_len;
...
static bool wpa_scan_res_ok(struct wpa_supplicant *wpa_s, struct wpa_ssid *ssid,
                const uint8_t *match_ssid, size_t match_ssid_len,
...
{
    int *freqs;
    uint16_t missing_links = 0, removed_links;
    uint8_t ap_mld_id;
...
static int wpas_get_snr_signal_info(uint32_t frequency, int avg_signal, int noise)
{
...
int wpa_supplicant_need_to_roam_within_ess(struct wpa_supplicant *wpa_s,
        struct wpa_bss *current_bss, struct wpa_bss *selected)
{
    int min_diff, diff;
    ...
    int cur_snr = 0;
    int ret = 0;
    const uint8_t *cur_ies = wpa_bss_ie_ptr(current_bss);
    const uint8_t *sel_ies = wpa_bss_ie_ptr(selected);
    ...
    // FIXME: 小数点を使うと'-mgeneral-regs-only' is incompatible with the use of floating-point types エラー。
    // 以下のようにするとこの行のエラー はなくなるが関数全体に対するエラーが
    // 消えないので当面このコンパイルオプションを外して対処した
    if (cur_est > sel_est * 1.5)    // cur_est * 10 > sel_est * 15
        min_diff += 10;
...
static int wpas_qos_map_set(struct wpa_supplicant *wpa_s, const uint8_t *qos_map,
                size_t len)
{
    int res;
...
{
    const uint8_t *bss_wpa = NULL, *bss_rsn = NULL, *bss_rsnx = NULL;
                    ...
                    struct ml_sta_link_info *ml_info)
{
    ...
    pos = ((uint8_t *) common_info) + common_info->len;
    ml_len -= sizeof(*ml) + common_info->len;
    while (ml_len > 2 && i < MAX_NUM_MLD_LINKS) {
        uint8_t sub_elem_len = pos[1];
        uint8_t sta_info_len;
        uint8_t nstr_bitmap_len = 0;
        uint16_t ctrl;
        const uint8_t *end;
...
static int wpa_sm_set_ml_info(struct wpa_supplicant *wpa_s)
{
    ...
    const uint8_t *bss_rsn = NULL, *bss_rsnx = NULL;
    int i;
    ...
    for_each_link(drv_mlo.req_links, i) {
        ...
        wpa_mlo.links[i].ap_rsne = bss_rsn ? (uint8_t *) bss_rsn : NULL;
        wpa_mlo.links[i].ap_rsne_len = bss_rsn ? 2 + bss_rsn[1] : 0;
        wpa_mlo.links[i].ap_rsnxe = bss_rsnx ? (uint8_t *) bss_rsnx : NULL;
                       ...
                       union wpa_event_data *data)
{
    ...
    if (wpa_key_mgmt_fils(wpa_s->key_mgmt)) {
        const uint8_t *fils_cache_id = wpa_bss_get_fils_cache_id(bss);
...
static int disconnect_reason_recoverable(uint16_t reason_code)
{
...
static int could_be_psk_mismatch(struct wpa_supplicant *wpa_s, uint16_t reason_code,
                 int locally_generated)
{
...
#ifdef CONFIG_IEEE80211R
static void ft_rx_action(struct wpa_supplicant *wpa_s, const uint8_t *data,
             size_t len)
{
    const uint8_t *sta_addr, *target_ap_addr;
    uint16_t status;
...
static void wpas_event_disconnect(struct wpa_supplicant *wpa_s, const uint8_t *addr,
                  uint16_t reason_code, int locally_generated,
                  const uint8_t *ie, size_t ie_len, int deauth)
{
#ifdef CONFIG_AP
...
static void wpas_event_rx_mgmt_action(struct wpa_supplicant *wpa_s,
                      const uint8_t *frame, size_t len, int freq,
                      int rssi)
{
...
static void wpas_event_assoc_reject(struct wpa_supplicant *wpa_s,
                    union wpa_event_data *data)
{
    const uint8_t *bssid = data->assoc_reject.bssid;
    struct ieee802_11_elems elems;
    struct ml_sta_link_info ml_info[MAX_NUM_MLD_LINKS];
    const uint8_t *link_bssids[MAX_NUM_MLD_LINKS + 1];
#ifdef CONFIG_MBO
                     ...
                     struct unprot_beacon *data)
{
    ...
    wpabuf_put_uint8_t(buf, WLAN_ACTION_WNM);
    wpabuf_put_uint8_t(buf, WNM_NOTIFICATION_REQ);
    wpabuf_put_uint8_t(buf, 1); /* Dialog Token */
    wpabuf_put_uint8_t(buf, WNM_NOTIF_TYPE_BEACON_PROTECTION_FAILURE);
...
static const char * bitmap_to_str(uint8_t value, char *buf)
{
    char *pos = buf;
    int i, k = 0;
