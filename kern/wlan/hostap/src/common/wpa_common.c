...
/**
 * wpa_pmk_to_ptk - PMK、アドレス、ノンスからPTKを計算する
...
#define MAX_Z_LEN 66 /* with NIST P-521 */
    uint8_t data[2 * ETH_ALEN + 2 * WPA_NONCE_LEN + MAX_Z_LEN];
    size_t data_len = 2 * ETH_ALEN + 2 * WPA_NONCE_LEN;
    uint8_t tmp[WPA_KCK_MAX_LEN + WPA_KEK_MAX_LEN + WPA_TK_MAX_LEN +
        WPA_KDK_MAX_LEN];
    size_t ptk_len;
#ifdef CONFIG_OWE
...
int fils_pmk_to_ptk(const uint8_t *pmk, size_t pmk_len, const uint8_t *spa, const uint8_t *aa,
            ...
            uint8_t *fils_ft, size_t *fils_ft_len, size_t kdk_len)
{
    uint8_t *data, *pos;
    size_t data_len;
    uint8_t tmp[FILS_ICK_MAX_LEN + WPA_KEK_MAX_LEN + WPA_TK_MAX_LEN +
...
#ifdef CONFIG_IEEE80211R
int wpa_ft_mic(int key_mgmt, const uint8_t *kck, size_t kck_len, const uint8_t *sta_addr,
           ...
           const uint8_t *ric, size_t ric_len,
...
static int wpa_ft_parse_ftie(const uint8_t *ie, size_t ie_len,
                 struct wpa_ft_ies *parse, const uint8_t *opt)
{
...
static int wpa_ft_parse_fte(int key_mgmt, const uint8_t *ie, size_t len,
                struct wpa_ft_ies *parse)
{
    size_t mic_len;
    uint8_t mic_len_info;
 ...
 * Returns: 0 on success, -1 on failure
 */
int pasn_pmk_to_ptk(const uint8_t *pmk, size_t pmk_len,
            const uint8_t *spa, const uint8_t *bssid,
            const uint8_t *dhss, size_t dhss_len,
            struct wpa_ptk *ptk, int akmp, int cipher,
            size_t kdk_len)
{
    uint8_t tmp[WPA_KCK_MAX_LEN + WPA_TK_MAX_LEN + WPA_KDK_MAX_LEN];
    uint8_t *data;
 ...
 */
uint8_t pasn_mic_len(int akmp, int cipher)
{
    if (pasn_use_sha384(akmp, cipher))
        return 24;
 ...
 * Returns: 0 on success, -1 on failure
 */
int pasn_mic(const uint8_t *kck, int akmp, int cipher,
         const uint8_t *addr1, const uint8_t *addr2,
         const uint8_t *data, size_t data_len,
         const uint8_t *frame, size_t frame_len, uint8_t *mic)
{
    uint8_t *buf;
    uint8_t hash[SHA384_MAC_LEN];
 ...
 * Returns: 0 on success, -1 on failure
 */
int pasn_auth_frame_hash(int akmp, int cipher, const uint8_t *data, size_t len,
             uint8_t *hash)
{
    if (pasn_use_sha384(akmp, cipher)) {
             ...
             struct wpa_ie_data *data)
{
    ...
    if (left >= 2) {
        uint16_t num_pmkid = WPA_GET_LE16(pos);
        pos += 2;
        left -= 2;
...
int wpa_derive_pmk_r0(const uint8_t *xxkey, size_t xxkey_len,
              ...
              int key_mgmt)
{
    uint8_t buf[1 + SSID_MAX_LEN + MOBILITY_DOMAIN_ID_LEN + 1 +
           FT_R0KH_ID_MAX_LEN + ETH_ALEN];
    uint8_t *pos, r0_key_data[64 + 16], hash[64];
    const uint8_t *addr[2];
    size_t len[2];
...
int wpa_derive_pmk_r1_name(const uint8_t *pmk_r0_name, const uint8_t *r1kh_id,
               const uint8_t *s1kh_id, uint8_t *pmk_r1_name,
               size_t pmk_r1_len)
{
    uint8_t hash[64];
...
{
    uint8_t buf[FT_R1KH_ID_LEN + ETH_ALEN];
    uint8_t *pos;
    int res;
 ...
 */
int wpa_pmk_r1_to_ptk(const uint8_t *pmk_r1, size_t pmk_r1_len,
              const uint8_t *snonce, const uint8_t *anonce,
              const uint8_t *sta_addr, const uint8_t *bssid,
              const uint8_t *pmk_r1_name,
              struct wpa_ptk *ptk, uint8_t *ptk_name, int akmp, int cipher,
              size_t kdk_len)
{
    uint8_t buf[2 * WPA_NONCE_LEN + 2 * ETH_ALEN];
    uint8_t *pos, hash[32];
    const uint8_t *addr[6];
    size_t len[6];
    uint8_t tmp[2 * WPA_KCK_MAX_LEN + 2 * WPA_KEK_MAX_LEN + WPA_TK_MAX_LEN +
           WPA_KDK_MAX_LEN];
...
int wpa_insert_pmkid(uint8_t *ies, size_t *ies_len, const uint8_t *pmkid, bool replace)
{
    uint8_t *start, *end, *rpos, *rend;
    int added = 0;
...
int rsn_cipher_put_suites(uint8_t *start, int ciphers)
{
    uint8_t *pos = start;
...
int wpa_cipher_put_suites(uint8_t *start, int ciphers)
{
    uint8_t *pos = start;
 ...
 */
static void wpa_parse_vendor_specific(const uint8_t *pos, const uint8_t *end,
 ...
 */
static int wpa_parse_generic(const uint8_t *pos, struct wpa_eapol_ie_parse *ie)
{
    uint8_t len = pos[1];
    size_t dlen = 2 + len;
    uint32_t selector;
 ...
 */
void wpa_pasn_build_auth_header(struct wpabuf *buf, const uint8_t *bssid,
                const uint8_t *src, const uint8_t *dst,
                uint8_t trans_seq, uint16_t status)
{
 ...
 * @cipher: The cipher suite
 */
int wpa_pasn_add_rsne(struct wpabuf *buf, const uint8_t *pmkid, int akmp, int cipher)
{
 ...
 */
void wpa_pasn_add_parameter_ie(struct wpabuf *buf, uint16_t pasn_group,
                   ...
                   const struct wpabuf *comeback, int after)
{
    ...
    if (comeback) {
        ...
        wpabuf_put_uint8_t(buf, wpabuf_len(comeback));
    ...
    if (pubkey) {
         ...
         */
        wpabuf_put_uint8_t(buf, wpabuf_len(pubkey) + 1);
        wpabuf_put_uint8_t(buf, compressed ? WPA_PASN_PUBKEY_COMPRESSED_0 :
                  ...
                  struct wpabuf *wrapped_data_buf)
{
    ...
    data = wpabuf_head_uint8_t(wrapped_data_buf);
    ...
    wpabuf_put_uint8_t(buf, WLAN_EID_EXTENSION);
...
{
    uint16_t capab = WPA_CAPABILITY_MFPC | WPA_CAPABILITY_MFPR;
 ...
 */
int wpa_pasn_parse_parameter_ie(const uint8_t *data, uint8_t len, bool from_ap,
                struct wpa_pasn_params_data *pasn_params)
{
    struct pasn_parameter_ie *params = (struct pasn_parameter_ie *) data;
    const uint8_t *pos = (const uint8_t *) (params + 1);
...
void wpa_pasn_add_rsnxe(struct wpabuf *buf, uint16_t capab)
{
    size_t flen;
    ...
    wpabuf_put_uint8_t(buf, WLAN_EID_RSNX);
    wpabuf_put_uint8_t(buf, flen);
    wpabuf_put_uint8_t(buf, capab & 0x00ff);
    capab >>= 8;
    if (capab)
        wpabuf_put_uint8_t(buf, capab);
}
...
int wpa_pasn_add_extra_ies(struct wpabuf *buf, const uint8_t *extra_ies, size_t len)
{
