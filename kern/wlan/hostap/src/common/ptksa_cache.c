...
ptksa_cache_get(struct ptksa_cache *ptksa, const uint8_t *addr, uint32_t cipher)
{
	return NULL;
}
...
struct ptksa_cache_entry *
ptksa_cache_add(struct ptksa_cache *ptksa, const uint8_t *own_addr, const uint8_t *addr,
		uint32_t cipher, uint32_t life_time, const struct wpa_ptk *ptk,
		void (*cb)(struct ptksa_cache_entry *e), void *ctx, uint32_t akmp)
{
	return NULL;
}
