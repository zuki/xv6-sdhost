...
void get_pri_sec_chan(struct wpa_scan_res *bss, int *pri_chan, int *sec_chan)
{
	...
	*pri_chan = *sec_chan = 0;
	...
	if (ieee802_11_parse_elems((uint8_t *) (bss + 1), bss->ie_len, &elems, 0) !=
		    ...
		    int sec_chan)
{
	...
	for (i = 0; i < scan_res->num; i++) {
		...
		if (ieee802_11_parse_elems((uint8_t *) (bss + 1), bss->ie_len,
					   &elems, 0) != ParseFailed &&
...
static void punct_update_legacy_bw_80(uint8_t bitmap, uint8_t pri_chan, uint8_t *seg0)
{
	uint8_t first_chan = *seg0 - 6, sec_chan;
...
static void punct_update_legacy_bw_160(uint8_t bitmap, uint8_t pri,
				       enum oper_chan_width *width, uint8_t *seg0)
{
	if (pri < *seg0) {
		*seg0 -= 8;
		if (bitmap & 0x0F) {
...
void punct_update_legacy_bw(uint16_t bitmap, uint8_t pri, enum oper_chan_width *width,
			    uint8_t *seg0, uint8_t *seg1)
{
...
int hostapd_set_freq_params(struct hostapd_freq_params *data,
			    ...
			    uint8_t edmg_channel, int ht_enabled,
			    ...
			    uint16_t punct_bitmap)
{
	...
	uint8_t seg0_legacy, seg1_legacy;
	...
	if (is_6ghz_freq(freq)) {
		...
		if (!center_segment0) {
			...
			data->bandwidth = (1 << (uint8_t) bw) * 20;
...
static int _ieee80211ac_cap_check(uint32_t hw, uint32_t conf, uint32_t cap,
				  const char *name)
{
	uint32_t req_cap = conf & cap;
...
static int ieee80211ac_cap_check_max(uint32_t hw, uint32_t conf, uint32_t mask,
				     unsigned int shift,
				     const char *name)
{
	uint32_t hw_max = hw & mask;
	uint32_t conf_val = conf & mask;
 ...
 * transmissions */
static const uint16_t punct_bitmap_80[] = { 0xF, 0xE, 0xD, 0xB, 0x7 };
static const uint16_t punct_bitmap_160[] = {
	0xFF, 0xFE, 0xFD, 0xFB, 0xF7, 0xEF, 0xDF, 0xBF,
	0x7F, 0xFC, 0xF3, 0xCF, 0x3F
};
static const uint16_t punct_bitmap_320[] = {
