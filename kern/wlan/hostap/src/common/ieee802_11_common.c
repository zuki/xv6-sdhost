...
static int ieee802_11_parse_vendor_specific(const uint8_t *pos, size_t elen,
...
static int ieee802_11_parse_mle(const uint8_t *pos, size_t elen, size_t **total_len,
				struct ieee802_11_elems *elems,
				int show_errors)
{
	uint8_t mle_type = pos[0] & MULTI_LINK_CONTROL_TYPE_MASK;
...
static int ieee802_11_parse_extension(const uint8_t *pos, size_t elen,
				      struct ieee802_11_elems *elems,
				      const uint8_t *start, size_t len,
				      int show_errors)
{
...
static ParseRes __ieee802_11_parse_elems(const uint8_t *start, size_t len,
					 struct ieee802_11_elems *elems,
					 int show_errors)
{
	const struct element *elem;
					 ...
					 uint8_t link_id, bool show_errors)
{
	...
	while (len > 2) {
		...
		size_t sta_info_len;
		uint16_t link_info_control;
		const uint8_t *non_inherit;
		...
		if (non_inherit && non_inherit[1] > 1) {
			uint8_t non_inherit_len = non_inherit[1] - 1;
...
static int ieee80211_chan_to_freq_us(uint8_t op_class, uint8_t chan)
{
	switch (op_class) {
...
static int ieee80211_chan_to_freq_eu(uint8_t op_class, uint8_t chan)
{
	switch (op_class) {
...
static int ieee80211_chan_to_freq_jp(uint8_t op_class, uint8_t chan)
{
...
static int ieee80211_chan_to_freq_cn(uint8_t op_class, uint8_t chan)
{
	switch (op_class) {
...
static int ieee80211_chan_to_freq_global(uint8_t op_class, uint8_t chan)
{
...
static int is_11b(uint8_t rate)
{
...
{
	uint16_t stype = WLAN_FC_GET_STYPE(fc);
...
struct wpabuf * mb_ies_by_info(struct mb_ies_info *info)
{
	...
	if (info->nof_ies) {
		...
		if (mb_ies) {
			for (i = 0; i < info->nof_ies; i++) {
				wpabuf_put_uint8_t(mb_ies, WLAN_EID_MULTI_BAND);
				wpabuf_put_uint8_t(mb_ies, info->ies[i].ie_len);
...
static uint8_t
global_op_class_from_country_array(uint8_t op_class, size_t array_size,
...
{
	struct wpabuf *buf;
	const uint8_t *pos, *end = data + len;
