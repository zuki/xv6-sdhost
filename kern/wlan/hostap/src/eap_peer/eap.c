 ...
 * Returns: 1 = allowed EAP method, 0 = not allowed
 */
int eap_allowed_method(struct eap_sm *sm, int vendor, uint32_t method)
{
...
int eap_peer_update_erp_next_seq_num(struct eap_sm *sm, uint16_t next_seq_num)
{
	struct eap_erp_key *erp;
	char *home_realm;
			  ...
			  size_t *rrk_len)
{
	...
	*username = (uint8_t *) erp->keyname_nai;

	pos++;
	*realm_len = os_strlen(pos);
	*realm = (uint8_t *) pos;

	*erp_next_seq_num = (uint16_t) erp->next_seq;
		       ...
		       size_t ext_emsk_len)
{
	...
	uint8_t EMSKname[EAP_EMSK_NAME_LEN];
	uint8_t len[2], ctx[3];
	char *realm;
...
struct wpabuf * eap_peer_build_erp_reauth_start(struct eap_sm *sm, uint8_t eap_id)
{
	...
	wpabuf_put_uint8_t(msg, 0x20); /* Flags: R=0 B=0 L=1 */
	wpabuf_put_be16(msg, erp->next_seq);

	wpabuf_put_uint8_t(msg, EAP_ERP_TLV_KEYNAME_NAI);
	wpabuf_put_uint8_t(msg, os_strlen(erp->keyname_nai));
	wpabuf_put_str(msg, erp->keyname_nai);

	wpabuf_put_uint8_t(msg, EAP_ERP_CS_HMAC_SHA256_128); /* Cryptosuite */
...
static int eap_peer_erp_reauth_start(struct eap_sm *sm, uint8_t eap_id)
{
	struct wpabuf *msg;
	...
	for (m = methods; m; m = m->next) {
		...
		if (eap_allowed_method(sm, m->vendor, m->method)) {
			wpa_printf(MSG_DEBUG, "EAP: allowed type: "
				   "vendor=%u method=%u",
				   m->vendor, m->method);
			wpabuf_put_uint8_t(resp, EAP_TYPE_EXPANDED);
	...
	for (m = methods; m; m = m->next) {
		...
		if (eap_allowed_method(sm, m->vendor, m->method)) {
			if (m->vendor != EAP_VENDOR_IETF) {
				...
				wpabuf_put_uint8_t(resp, m->method);
			found++;
		}
	}
	if (!found)
		wpabuf_put_uint8_t(resp, EAP_TYPE_NONE);
				...
				struct eap_peer_config *conf)
{
	...
	wpa_hexdump_ascii(MSG_DEBUG, "IMSI", (uint8_t *) imsi, imsi_len);
	...
	wpa_hexdump_ascii(MSG_DEBUG, "IMSI + realm", (uint8_t *) imsi, imsi_len);
...
struct wpabuf * eap_sm_buildIdentity(struct eap_sm *sm, int id, int encrypted)
{
	...
	if (sm->m && sm->m->get_identity &&
		...
		const uint8_t *pos = config->identity;
		const uint8_t *end = config->identity + config->identity_len;
		...
		if (*pos == '0' || *pos == '1' || *pos == '6')
			wpabuf_put_uint8_t(privacy_identity, *pos);
...
void eap_peer_finish(struct eap_sm *sm, const struct eap_hdr *hdr, size_t len)
{
	...
	if (hmac_sha256(erp->rIK, erp->rIK_len, (const uint8_t *) hdr,
			end - ((const uint8_t *) hdr) - hash_len, hash) < 0)
		return;
...
{
	int v;
	uint32_t type = eap_peer_get_type(name, &v);
