...
static const uint8_t null_rsc[8] = { 0, 0, 0, 0, 0, 0, 0, 0 };


static void _wpa_hexdump_link(int level, uint8_t link_id, const char *title,
...
static void wpa_hexdump_link(int level, uint8_t link_id, const char *title,
...
static void wpa_hexdump_link_key(int level, uint8_t link_id, const char *title,
                 const void *buf, size_t len)
{
               ...
               uint8_t *msg, size_t msg_len, uint8_t *key_mic)
{
    ...
    if (mic_len) {
        ...
        uint8_t *buf, *s_key_data, *key_data;
        ...
        WPA_PUT_BE16((uint8_t *) (key + 1), AES_BLOCK_SIZE + key_data_len);

        s_hdr = (struct ieee802_1x_hdr *) msg;
        s_key = (struct wpa_eapol_key *) (s_hdr + 1);
        s_key_data = ((uint8_t *) (s_key + 1)) + 2;
                  ...
                  const uint8_t *pmkid)
{
    ...
    if (pmkid && !sm->cur_pmksa) {
        /* RSN IEを生成するドライバを使用する場合、wpa_supplicantは
         * この1/4メッセージを受信する前にアソシエーション情報イベントを
         * 取得する時間が十分ない可能性があるため、ここで一致するPMKSA
         * キャッシュエントリを検索する */
    ...
    if (pmkid && sm->cur_pmksa &&
        ...
        if (res == 0 &&
            ...
            if (sm->proto == WPA_PROTO_RSN &&
                ...
                const uint8_t *fils_cache_id = NULL;
                   ...
                   struct wpa_ptk *ptk)
{
    ...
    struct wpa_eapol_key *reply;
    uint8_t *rbuf, *key_mic;
    uint8_t *rsn_ie_buf = NULL;
    uint16_t key_info;
...
static uint8_t * rsn_add_kde(uint8_t *pos, uint32_t kde, const uint8_t *data, size_t data_len)
{
...
static uint8_t * wpa_mlo_link_kde(struct wpa_sm *sm, uint8_t *pos)
{
    int i;
    uint8_t hdr[1 + ETH_ALEN];
...
static bool is_valid_ap_mld_mac_kde(struct wpa_sm *sm, const uint8_t *mac_kde)
{
    return mac_kde &&
...
static void wpa_supplicant_process_1_of_4_wpa(struct wpa_sm *sm,
                          const unsigned char *src_addr,
                          const struct wpa_eapol_key *key,
                          uint16_t ver, const uint8_t *key_data,
                          size_t key_data_len,
...
static int wpa_supplicant_install_gtk(struct wpa_sm *sm,
                      ...
                      const uint8_t *key_rsc, int wnm_sleep)
{
    const uint8_t *_gtk = gd->gtk;
    uint8_t gtk_buf[32];
...
static int wpa_supplicant_install_mlo_gtk(struct wpa_sm *sm, uint8_t link_id,
                      const struct wpa_gtk_data *gd,
                      const uint8_t *key_rsc, int wnm_sleep)
{
    const uint8_t *gtk = gd->gtk;
    uint8_t gtk_buf[32];
...
static int wpa_supplicant_mlo_gtk(struct wpa_sm *sm, uint8_t link_id, const uint8_t *gtk,
                       ...
                       int wnm_sleep)
{
    ...
    uint16_t keyidx = WPA_GET_LE16(igtk->keyid);
...
{
    size_t len = wpa_cipher_key_len(sm->mgmt_group_cipher);
    uint16_t keyidx = WPA_GET_LE16(bigtk->keyid);
...
static int wpa_supplicant_install_mlo_igtk(struct wpa_sm *sm, uint8_t link_id,
...
static int
wpa_supplicant_install_mlo_bigtk(struct wpa_sm *sm, uint8_t link_id,
...
static int _mlo_ieee80211w_set_keys(struct wpa_sm *sm, uint8_t link_id,
                    struct wpa_eapol_ie_parse *ie)
{
    size_t len;
...
static void wpa_report_ie_mismatch(struct wpa_sm *sm,
                   const char *reason, const uint8_t *src_addr,
                   const uint8_t *wpa_ie, size_t wpa_ie_len,
...
static int wpa_supplicant_validate_ie_ft(struct wpa_sm *sm,
                     ...
                     struct wpa_eapol_ie_parse *ie)
{
    const uint8_t *pos, *end, *mdie = NULL, *ftie = NULL;
...
static int wpa_supplicant_validate_link_kde(struct wpa_sm *sm, uint8_t link_id,
                        const uint8_t *link_kde,
                        size_t link_kde_len)
{
    size_t rsne_len = 0, rsnxe_len = 0;
    const uint8_t *rsne = NULL, *rsnxe = NULL;
...
static void wpa_supplicant_process_mlo_1_of_2(struct wpa_sm *sm,
                          ...
                          size_t key_data_len, uint16_t ver)
{
    uint16_t key_info;
    uint8_t i;
    struct wpa_eapol_ie_parse ie;
...
{
    uint8_t mic[WPA_EAPOL_KEY_MIC_MAX_LEN];
    int ok = 0;
    size_t mic_len = wpa_mic_len(sm->key_mgmt, sm->pmk_len);
                       ...
                       uint8_t *key_data, size_t *key_data_len)
{
    ...
    continue_fuzz:
        ...
        bin_clear_free(buf, *key_data_len);
        WPA_PUT_BE16(((uint8_t *) (key + 1)) + mic_len, *key_data_len);
    } else {
        wpa_msg(sm->ctx->msg_ctx, MSG_WARNING,
            "WPA: Unsupported key_info type %d", ver);
        return -1;
...
static void wpa_eapol_key_dump(struct wpa_sm *sm,
                   const struct wpa_eapol_key *key,
                   unsigned int key_data_len,
                   const uint8_t *mic, unsigned int mic_len)
{
#ifndef CONFIG_NO_STDOUT_DEBUG
    uint16_t key_info = WPA_GET_BE16(key->key_info);
...
#ifdef CONFIG_FILS
static int wpa_supp_aead_decrypt(struct wpa_sm *sm, uint8_t *buf, size_t buf_len,
                 size_t *key_data_len)
{
    struct wpa_ptk *ptk;
...
static int wpa_sm_rx_eapol_wpa(struct wpa_sm *sm, const uint8_t *src_addr,
                   struct wpa_eapol_key *key,
                   enum frame_encryption encrypted,
                   const uint8_t *tmp, size_t data_len,
                   uint8_t *key_data, size_t key_data_len)
{
    uint16_t key_info, ver;
...
static uint32_t wpa_key_mgmt_suite(struct wpa_sm *sm)
{
    switch (sm->key_mgmt) {
...
{
    uint16_t keyinfo;
    uint8_t keylen;  /* plaintext key len */
    uint8_t *key_rsc;
...
#if 0
    if (sm->mlo.valid_links)
        wpa_printf(MSG_DEBUG, "ap_mld_addr: " MACSTR, MAC2STR(sm->mlo.ap_mld_addr));
    else
        wpa_printf(MSG_DEBUG, "sm_bssid: " MACSTR, MAC2STR(sm->bssid));
#endif
...
struct wpabuf * fils_build_auth(struct wpa_sm *sm, int dh_group, const uint8_t *md)
{
    ...
    if (md) {
        ...
        wpabuf_put_uint8_t(buf, WLAN_EID_MOBILITY_DOMAIN);
        wpabuf_put_uint8_t(buf, sizeof(*mdie));
    ...
    /* FILS Nonce */
    wpabuf_put_uint8_t(buf, WLAN_EID_EXTENSION); /* Element ID */
    wpabuf_put_uint8_t(buf, 1 + FILS_NONCE_LEN); /* Length */
    /* Element ID Extension */
    wpabuf_put_uint8_t(buf, WLAN_EID_EXT_FILS_NONCE);
    wpabuf_put_data(buf, sm->fils_nonce, FILS_NONCE_LEN);

    /* FILS Session */
    wpabuf_put_uint8_t(buf, WLAN_EID_EXTENSION); /* Element ID */
    wpabuf_put_uint8_t(buf, 1 + FILS_SESSION_LEN); /* Length */
    /* Element ID Extension */
    wpabuf_put_uint8_t(buf, WLAN_EID_EXT_FILS_SESSION);
    ...
    if (erp_msg) {
        wpabuf_put_uint8_t(buf, WLAN_EID_EXTENSION); /* Element ID */
        wpabuf_put_uint8_t(buf, 1 + wpabuf_len(erp_msg)); /* Length */
        /* Element ID Extension */
        wpabuf_put_uint8_t(buf, WLAN_EID_EXT_WRAPPED_DATA);
              ...
              size_t len)
{
    ...
    const uint8_t *g_sta = NULL;
    size_t g_sta_len = 0;
    const uint8_t *g_ap = NULL;
...
static int fils_ft_build_assoc_req_rsne(struct wpa_sm *sm, struct wpabuf *buf)
{
    ...
    rsnie->len = ((uint8_t *) wpabuf_put(buf, 0) - (uint8_t *) rsnie) - 2;
    return 0;
}
#endif /* CONFIG_IEEE80211R */
                     ...
                     unsigned int num_hlp)
{
    ...
    /* FILS Key Confirm */
    wpabuf_put_uint8_t(buf, WLAN_EID_EXTENSION); /* Element ID */
    wpabuf_put_uint8_t(buf, 1 + sm->fils_key_auth_len); /* Length */
    /* Element ID Extension */
    wpabuf_put_uint8_t(buf, WLAN_EID_EXT_FILS_KEY_CONFIRM);
    wpabuf_put_data(buf, sm->fils_key_auth_sta, sm->fils_key_auth_len);

    /* FILS HLP Container */
    for (i = 0; hlp && i < num_hlp; i++) {
        const uint8_t *pos = wpabuf_head(hlp[i]);
        ...
        if (left <= 254)
            ...
            len = 255;
        wpabuf_put_uint8_t(buf, len); /* Length */
        /* Element ID Extension */
        wpabuf_put_uint8_t(buf, WLAN_EID_EXT_FILS_HLP_CONTAINER);
        ...
        left -= len - 1;
        while (left) {
            wpabuf_put_uint8_t(buf, WLAN_EID_FRAGMENT);
            len = left > 255 ? 255 : left;
            wpabuf_put_uint8_t(buf, len);
...
static void fils_process_hlp_resp(struct wpa_sm *sm, const uint8_t *resp, size_t len)
{
    const uint8_t *pos, *end;
...
static void fils_process_hlp_container(struct wpa_sm *sm, const uint8_t *pos,
                       size_t len)
{
...
struct wpabuf * owe_build_assoc_req(struct wpa_sm *sm, uint16_t group)
{
    ...
    if (!ie)
        goto fail;
    wpabuf_put_uint8_t(ie, WLAN_EID_EXTENSION);
    wpabuf_put_uint8_t(ie, 1 + 2 + wpabuf_len(pub));
    wpabuf_put_uint8_t(ie, WLAN_EID_EXT_OWE_DH_PARAM);
               ...
               const uint8_t *resp_ies, size_t resp_ies_len)
{
    ...
    int res;
    uint8_t prk[SHA512_MAC_LEN], pmkid[SHA512_MAC_LEN];
    ...
    if (group == 19)
        res = hmac_sha256_kdf(prk, hash_len, NULL, (const uint8_t *) info,
                      os_strlen(info), sm->pmk, hash_len);
    else if (group == 20)
        res = hmac_sha384_kdf(prk, hash_len, NULL, (const uint8_t *) info,
                      os_strlen(info), sm->pmk, hash_len);
    else if (group == 21)
        res = hmac_sha512_kdf(prk, hash_len, NULL, (const uint8_t *) info,
                      os_strlen(info), sm->pmk, hash_len);
