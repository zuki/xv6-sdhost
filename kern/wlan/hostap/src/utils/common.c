...
void wpa_get_ntp_timestamp(uint8_t *buf)
{
	...
	os_memcpy(buf, (uint8_t *) &tmp, 4);
	tmp = host_to_be32(usec);
	os_memcpy(buf + 4, (uint8_t *) &tmp, 4);
}
...
int wpa_scnprintf(char *buf, size_t size, const char *fmt, ...)
{
	...
	va_start(ap, fmt);
	ret = vsnprintfmt(buf, size, fmt, ap);
	va_end(ap);
...
static inline int _wpa_snprintf_hex(char *buf, size_t buf_size, const uint8_t *data,
...
char * wpa_config_parse_string(const char *value, size_t *len)
{
	if (*value == '"') {
		...
		*len = printf_decode((uint8_t *) str, tlen + 1, tstr);
		os_free(tstr);
