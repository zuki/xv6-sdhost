...
static void eapol_sm_txLogoff(struct eapol_sm *sm)
{
    ...
    sm->ctx->eapol_send(sm->ctx->eapol_send_ctx,
                IEEE802_1X_TYPE_EAPOL_LOGOFF, (uint8_t *) "", 0);
...
static void eapol_sm_txStart(struct eapol_sm *sm)
{
    ...
    sm->ctx->eapol_send(sm->ctx->eapol_send_ctx,
                IEEE802_1X_TYPE_EAPOL_START, (uint8_t *) "", 0);
...
struct eap_key_data {
    uint8_t encr_key[IEEE8021X_ENCR_KEY_LEN];
    uint8_t sign_key[IEEE8021X_SIGN_KEY_LEN];
};
...
static void eapol_sm_processKey(struct eapol_sm *sm)
{
    ...
    uint8_t orig_key_sign[IEEE8021X_KEY_SIGN_LEN], datakey[32];
#ifndef CONFIG_NO_RC4
    uint8_t ekey[IEEE8021X_KEY_IV_LEN + IEEE8021X_ENCR_KEY_LEN];
#endif /* CONFIG_NO_RC4 */
    int key_len, res, sign_key_len, encr_key_len;
    uint16_t rx_key_length;
    size_t plen;
    ...
    res = eapol_sm_get_key(sm, (uint8_t *) &keydata, sizeof(keydata));
    if (res < 0) {
    ...
    if (res == 16) {
        ...
        res = eapol_sm_get_key(sm, (uint8_t *) &keydata, 16);
        if (res) {
...
/**
 * eapol_sm_step - EAPOL ステートマシンのステップ関数
        ...
        if (sm->eapReqData) {
            ...
            if (sm->use_eap_proxy) {
                eap_proxy_packet_update(
                    sm->eap_proxy,
                    wpabuf_mhead_uint8_t(sm->eapReqData),
...
static void eapol_sm_set_anon_id(void *ctx, const uint8_t *id, size_t len)
{
    struct eapol_sm *sm = ctx;
