...
struct tcp_segment_info {
    uint32_t seq;           // セグメントシーケンス番号
    uint32_t ack;           // セグメント確認番号
    uint16_t len;           // セグメント長
    uint16_t wnd;           // セグメントウィンドウ
    uint16_t up;            // セグメント緊急ポインタ
};
...
struct tcp_pcb {
    ...
    struct {
        uint32_t nxt;       // 次の送信
        uint32_t una;       // 確認未の送信
        uint16_t wnd;       // ウィンドウの送信
        uint16_t up;        // 緊急ポインタの送信
        uint32_t wl1;       // 最後のウィンドウの更新に使用されるセグメントシーケンス番号
        uint32_t wl2;       // 最後のウィンドウの更新に使用されるセグメント確認番号
    } snd;
    uint32_t iss;           // 初期送信シーケンス番号
    struct {
        uint32_t nxt;       // 次の受信
        uint16_t wnd;       // ウィンドウの受信
        uint16_t up;        // 緊急ポインタの受信
    } rcv;
...
static void tcp_dump(const uint8_t *data, size_t len)
{
    ...
    cprintf("        seq: %u\n", ntoh32(hdr->seq));
    cprintf("        ack: %u\n", ntoh32(hdr->ack));
    cprintf("        off: 0x%02x (%d)\n", hdr->off, (hdr->off >> 4) << 2);
    cprintf("        flg: 0x%02x (%s)\n", hdr->flg, tcp_flg_ntoa(hdr->flg));
    cprintf("        wnd: %u\n", ntoh16(hdr->wnd));
    cprintf("        sum: 0x%04x\n", ntoh16(hdr->sum));
    cprintf("         up: %u\n", ntoh16(hdr->up));
...
static void tcp_input(const uint8_t *data, size_t len, ip_addr_t src, ip_addr_t dst, struct ip_iface *iface)
{
    ...
    if (src == IP_ADDR_BROADCAST || src == iface->broadcast || dst == IP_ADDR_BROADCAST || dst == iface->broadcast) {
        error("only supports unicast, src=%s, dst=%s",
...
int tcp_close(int id)
{
    ...
    case TCP_PCB_STATE_CLOSE_WAIT:
        ...
        break;
    default:
        error("unknown state '%u'", pcb->state);
        mutex_unlock(&mutex);
        return -1;
    }
...
int tcp_connect(int id, struct ip_endpoint *foreign)
{
    ...
    if (pcb->mode != TCP_PCB_MODE_SOCKET) {
        error("not opened in socket mode");
        mutex_unlock(&mutex);
        return -1;
    }
    ...
    if (local.addr == IP_ADDR_ANY) {
        ...
        if (!iface) {
            error("ip_route_get_iface() failure");
            mutex_unlock(&mutex);
            return -1;
        }
...
int tcp_bind(int id, struct ip_endpoint *local)
{
    ...
    if (exist) {
        error("already bound, exist=%s", ip_endpoint_ntop(&exist->local, ep, sizeof(ep)));
        mutex_unlock(&mutex);
        return -1;
    }
    pcb->local = *local;
