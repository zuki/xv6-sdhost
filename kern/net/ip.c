...
static struct ip_route *ip_route_add(ip_addr_t network, ip_addr_t netmask, ip_addr_t nexthop, struct ip_iface *iface)
{
    ...
    route->next = routes;
    routes = route;
    info("route added: network=%s, netmask=%s, nexthop=%s, iface=%s dev=%s",
...
int ip_route_set_default_gateway(struct ip_iface *iface, const char *gateway)
{
    ...
    if (ip_addr_pton(gateway, &gw) == -1) {
        error("ip_addr_pton() failure, addr=%s", gateway);
        return -1;
    }
    if (!ip_route_add(IP_ADDR_ANY, IP_ADDR_ANY, gw, iface)) {
        error("ip_route_add() failure");
        return -1;
    }
    return 0;
}
...
struct ip_iface *ip_iface_alloc(const char *unicast, const char *netmask)
{
    ...
    if (ip_addr_pton(unicast, &iface->unicast) == -1) {
        error("ip_addr_pton() failure, addr=%s", unicast);
        memory_free(iface);
        return NULL;
    }
    if (ip_addr_pton(netmask, &iface->netmask) == -1) {
        error("ip_addr_pton() failure, addr=%s", netmask);
        memory_free(iface);
        return NULL;
    }
...
int ip_iface_register(struct net_device *dev, struct ip_iface *iface)
{
    ...
    if (net_device_add_iface(dev, NET_IFACE(iface)) == -1) {
        error("net_device_add_iface() failure");
        return -1;
    }
    ...
    iface->next = ifaces;
    ifaces = iface;
    info("registered: dev=%s, unicast=%s, netmask=%s, broadcast=%s", dev->name,
...
int ip_protocol_register(uint8_t type, void (*handler)(const uint8_t *data, size_t len, ip_addr_t src, ip_addr_t dst, struct ip_iface *iface))
{
    ...
    for (entry = protocols; entry; entry = entry->next) {
        if (entry->type == type) {
            error("already exists, type=%u", type);
            return -1;
        }
    }
...
static void ip_input(const uint8_t *data, size_t len, struct net_device *dev)
{
    ...
    hlen = (hdr->vhl & 0x0f) << 2;
    if (len < hlen) {
        error("header length error: len=%zu < hlen=%u", len, hlen);
        return;
    }
    total = ntoh16(hdr->total);
    if (len < total) {
        error("total length error: len=%zu < total=%u", len, total);
        return;
    }
    if (cksum16((uint16_t *)hdr, hlen, 0) != 0) {
        error("checksum error: sum=0x%04x, verify=0x%04x", ntoh16(hdr->sum), ntoh16(cksum16((uint16_t *)hdr, hlen, -hdr->sum)));
        return;
    }
    offset = ntoh16(hdr->offset);
...
ssize_t ip_output(uint8_t protocol, const uint8_t *data, size_t len, ip_addr_t src, ip_addr_t dst)
{
    ...
    route = ip_route_lookup(dst);
    if (!route) {
        error("no route to host, dst=%s", ip_addr_ntop(dst, addr, sizeof(addr)));
        return -1;
    }
    iface = route->iface;
    if (src != IP_ADDR_ANY && src != iface->unicast) {
        error("unable to output with specified source address, src=%s", ip_addr_ntop(src, addr, sizeof(addr)));
        return -1;
    }
    nexthop = (route->nexthop != IP_ADDR_ANY) ? route->nexthop : dst;
    if (NET_IFACE(iface)->dev->mtu < IP_HDR_SIZE_MIN + len) {
        error("too long, dev=%s, mtu=%u < %zu",
