...
#ifndef INC_WPA_SUPPLICANT_WPA_SUPPLICANT_I_H
#define INC_WPA_SUPPLICANT_WPA_SUPPLICANT_I_H
...
struct ipv6_params {
    struct in6_addr *src_ip;        // FIXME: 元々はpointerではない
    struct in6_addr *dst_ip;        //   struct in6_addr の定義を検討
    uint16_t src_port;
    uint16_t dst_port;
    uint8_t dscp;
    uint8_t next_header;
...
struct wpa_supplicant {
    ...
    uint8_t mlo_assoc_link_id;
    uint16_t valid_links; /* bitmap of valid MLO link IDs */
    struct {
...
#define MAX_SCAN_ID 16
    ...
    uint8_t ml_probe_bssid[ETH_ALEN];
    int ml_probe_mld_id;
    uint16_t ml_probe_links;
    ...
    struct {
        ...
        uint8_t ext_auth_ap_mld_addr[ETH_ALEN];
        bool ext_ml_auth;
        int *sae_rejected_groups;
#endif /* CONFIG_SAE */
        uint16_t assoc_auth_type;
    } sme;
#endif /* CONFIG_SME */
...
#define MAC_ADDR_RAND_ALL        (MAC_ADDR_RAND_SCAN | \
    ...
    uint8_t wnm_dissoc_addr[ETH_ALEN];
    ...
    uint8_t last_tk_addr[ETH_ALEN];
    int last_tk_key_idx;
    uint8_t last_tk[WPA_TK_MAX_LEN];
    size_t last_tk_len;
    ...
    uint8_t dpp_intro_peer_version;
    ...
    uint8_t dpp_last_ssid[SSID_MAX_LEN];
    ...
    uint8_t dpp_pb_init_hash[SHA256_MAC_LEN];
    int dpp_pb_stop_iter;
    bool dpp_pb_discovery_done;
    uint8_t dpp_pb_c_nonce[DPP_MAX_NONCE_LEN];
    ...
    uint8_t dpp_pb_resp_hash[SHA256_MAC_LEN];
    ...
    uint8_t scs_dialog_token;
    struct dl_list active_scs_ids;
    bool ongoing_scs_req;
    uint8_t dscp_req_dialog_token;
    uint8_t dscp_query_dialog_token;
...
void wpa_supplicant_rx_eapol(void *ctx, const uint8_t *own_addr,
...
const uint8_t * wpas_mbo_check_assoc_disallow(struct wpa_bss *bss);
...
enum chan_allowed verify_channel(struct hostapd_hw_modes *mode, uint8_t op_class,
                 uint8_t channel, uint8_t bw);
size_t wpas_supp_op_class_ie(struct wpa_supplicant *wpa_s,
                 struct wpa_ssid *ssid,
                 struct wpa_bss *bss, uint8_t *pos, size_t len);
...
void wpas_handle_robust_av_recv_action(struct wpa_supplicant *wpa_s,
                       ...
                       size_t len);
void wpas_handle_assoc_resp_mscs(struct wpa_supplicant *wpa_s, const uint8_t *bssid,
...
int wpas_pasn_auth_start(struct wpa_supplicant *wpa_s, const uint8_t *own_addr,
             const uint8_t *bssid, int akmp, int cipher,
             uint16_t group, int network_id,
             const uint8_t *comeback, size_t comeback_len);
...
int wpas_pasn_deauthenticate(struct wpa_supplicant *wpa_s, const uint8_t *own_addr,
                 const uint8_t *bssid);
