...
#ifndef INC_WLAN_EAP_COMMON_IKEV2_COMMON_H
#define INC_WLAN_EAP_COMMON_IKEV2_COMMON_H
...
struct ikev2_hdr {
	uint8_t i_spi[IKEV2_SPI_LEN]; /* IKE_SA Initiator's SPI */
	uint8_t r_spi[IKEV2_SPI_LEN]; /* IKE_SA Responder's SPI */
	uint8_t next_payload;
	uint8_t version; /* MjVer | MnVer */
	uint8_t exchange_type;
	uint8_t flags;
	uint8_t message_id[4];
	uint8_t length[4]; /* total length of HDR + payloads */
} STRUCT_PACKED;

struct ikev2_payload_hdr {
	uint8_t next_payload;
	uint8_t flags;
	uint8_t payload_length[2]; /* this payload, including the payload header */
} STRUCT_PACKED;

struct ikev2_proposal {
	uint8_t type; /* 0 (last) or 2 (more) */
	uint8_t reserved;
	uint8_t proposal_length[2]; /* including all transform and attributes */
	uint8_t proposal_num;
	uint8_t protocol_id; /* IKEV2_PROTOCOL_* */
	uint8_t spi_size;
...
struct ikev2_transform {
	uint8_t type; /* 0 (last) or 3 (more) */
	uint8_t reserved;
	uint8_t transform_length[2]; /* including Header and Attributes */
	uint8_t transform_type;
	uint8_t reserved2;
	uint8_t transform_id[2];
...
struct ikev2_keys {
	uint8_t *SK_d, *SK_ai, *SK_ar, *SK_ei, *SK_er, *SK_pi, *SK_pr;
...
int ikev2_integ_hash(int alg, const uint8_t *key, size_t key_len, const uint8_t *data,
		     size_t data_len, uint8_t *hash);
const struct ikev2_prf_alg * ikev2_get_prf(int id);
int ikev2_prf_hash(int alg, const uint8_t *key, size_t key_len,
		   size_t num_elem, const uint8_t *addr[], const size_t *len,
		   uint8_t *hash);
int ikev2_prf_plus(int alg, const uint8_t *key, size_t key_len,
		   const uint8_t *data, size_t data_len,
		   uint8_t *out, size_t out_len);
const struct ikev2_encr_alg * ikev2_get_encr(int id);
int ikev2_encr_encrypt(int alg, const uint8_t *key, size_t key_len, const uint8_t *iv,
		       const uint8_t *plain, uint8_t *crypt, size_t len);
int ikev2_encr_decrypt(int alg, const uint8_t *key, size_t key_len, const uint8_t *iv,
		       const uint8_t *crypt, uint8_t *plain, size_t len);

int ikev2_derive_auth_data(int prf_alg, const struct wpabuf *sign_msg,
			   const uint8_t *ID, size_t ID_len, uint8_t ID_type,
			   struct ikev2_keys *keys, int initiator,
			   const uint8_t *shared_secret, size_t shared_secret_len,
			   const uint8_t *nonce, size_t nonce_len,
			   const uint8_t *key_pad, size_t key_pad_len,
			   uint8_t *auth_data);
...
struct ikev2_payloads {
	...
	uint8_t encr_next_payload;
	const uint8_t *notification;
	size_t notification_len;
};

int ikev2_parse_payloads(struct ikev2_payloads *payloads,
			 uint8_t next_payload, const uint8_t *pos, const uint8_t *end);

uint8_t * ikev2_decrypt_payload(int encr_id, int integ_id, struct ikev2_keys *keys,
...
int ikev2_build_encrypted(int encr_id, int integ_id, struct ikev2_keys *keys,
			  ...
			  struct wpabuf *plain, uint8_t next_payload);
int ikev2_derive_sk_keys(const struct ikev2_prf_alg *prf,
			 const struct ikev2_integ_alg *integ,
			 const struct ikev2_encr_alg *encr,
			 const uint8_t *skeyseed, const uint8_t *data, size_t data_len,
