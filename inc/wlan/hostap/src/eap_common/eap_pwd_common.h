...
#ifndef INC_WLAN_EAP_COMMON_EAP_PWD_COMMON_H
#define INC_WLAN_EAP_COMMON_EAP_PWD_COMMON_H
...
#define EAP_PWD_PREP_SSHA512            5
	uint8_t identity[0];     /* length inferred from payload */
} STRUCT_PACKED;

/* common routines */
EAP_PWD_group * get_eap_pwd_group(uint16_t num);
int compute_password_element(EAP_PWD_group *grp, uint16_t num,
...
int compute_keys(EAP_PWD_group *grp, const struct crypto_bignum *k,
		 ...
		 const uint8_t *confirm_peer, const uint8_t *confirm_server,
		 const uint32_t *ciphersuite, uint8_t *msk, uint8_t *emsk, uint8_t *session_id);
struct crypto_hash * eap_pwd_h_init(void);
void eap_pwd_h_update(struct crypto_hash *hash, const uint8_t *data, size_t len);
void eap_pwd_h_final(struct crypto_hash *hash, uint8_t *digest);
struct crypto_ec_point * eap_pwd_get_element(EAP_PWD_group *group,
					     const uint8_t *buf);
struct crypto_bignum * eap_pwd_get_scalar(EAP_PWD_group *group, const uint8_t *buf);
