...
#ifndef INC_WLAN_EAP_COMMON_EAP_TEAP_H
#define INC_WLAN_EAP_COMMON_EAP_TEAP_H
...
struct teap_tlv_crypto_binding {
	...
	uint8_t nonce[EAP_TEAP_NONCE_LEN];
	uint8_t emsk_compound_mac[EAP_TEAP_COMPOUND_MAC_LEN];
	uint8_t msk_compound_mac[EAP_TEAP_COMPOUND_MAC_LEN];
} STRUCT_PACKED;
...
struct eap_teap_tlv_parse {
	...
	uint8_t request_action_status;
	uint8_t *pac;
	size_t pac_len;
	uint8_t *basic_auth_req;
	size_t basic_auth_req_len;
	uint8_t *basic_auth_resp;
	size_t basic_auth_resp_len;
	uint32_t error_code;
	uint16_t identity_type;
};

void eap_teap_put_tlv_hdr(struct wpabuf *buf, uint16_t type, uint16_t len);
void eap_teap_put_tlv(struct wpabuf *buf, uint16_t type, const void *data, uint16_t len);
void eap_teap_put_tlv_buf(struct wpabuf *buf, uint16_t type,
			  const struct wpabuf *data);
struct wpabuf * eap_teap_tlv_eap_payload(struct wpabuf *buf);
int eap_teap_derive_eap_msk(uint16_t tls_cs, const uint8_t *simck, uint8_t *msk);
int eap_teap_derive_eap_emsk(uint16_t tls_cs, const uint8_t *simck, uint8_t *emsk);
int eap_teap_derive_cmk_basic_pw_auth(uint16_t tls_cs, const uint8_t *s_imck_msk,
				      uint8_t *cmk);
int eap_teap_derive_imck(uint16_t tls_cs,
			 const uint8_t *prev_s_imck_msk, const uint8_t *prev_s_imck_emsk,
			 const uint8_t *msk, size_t msk_len,
			 const uint8_t *emsk, size_t emsk_len,
			 uint8_t *s_imck_msk, uint8_t *cmk_msk,
			 uint8_t *s_imck_emsk, uint8_t *cmk_emsk);
int eap_teap_compound_mac(uint16_t tls_cs, const struct teap_tlv_crypto_binding *cb,
			  const struct wpabuf *server_outer_tlvs,
			  const struct wpabuf *peer_outer_tlvs,
			  const uint8_t *cmk, uint8_t *compound_mac);
int eap_teap_parse_tlv(struct eap_teap_tlv_parse *tlv,
		       int tlv_type, uint8_t *pos, size_t len);
...
int eap_teap_allowed_anon_prov_cipher_suite(uint16_t cs);
