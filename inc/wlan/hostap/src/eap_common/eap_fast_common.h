...
#ifndef INC_WLAN_EAP_COMMON_EAP_FAST_H
#define INC_WLAN_EAP_COMMON_EAP_FAST_H
...
struct eap_fast_key_block_provisioning {
	...
	uint8_t server_challenge[16]; /* MSCHAPv2 ServerChallenge */
	uint8_t client_challenge[16]; /* MSCHAPv2 ClientChallenge */
};
...
void eap_fast_put_tlv_hdr(struct wpabuf *buf, uint16_t type, uint16_t len);
void eap_fast_put_tlv(struct wpabuf *buf, uint16_t type, const void *data,
		      uint16_t len);
void eap_fast_put_tlv_buf(struct wpabuf *buf, uint16_t type,
			  const struct wpabuf *data);
struct wpabuf * eap_fast_tlv_eap_payload(struct wpabuf *buf);
void eap_fast_derive_master_secret(const uint8_t *pac_key, const uint8_t *server_random,
				   const uint8_t *client_random, uint8_t *master_secret);
uint8_t * eap_fast_derive_key(void *ssl_ctx, struct tls_connection *conn,
			 size_t len);
int eap_fast_derive_eap_msk(const uint8_t *simck, uint8_t *msk);
int eap_fast_derive_eap_emsk(const uint8_t *simck, uint8_t *emsk);
