...
#ifndef INC_WLAN_P2P_H
#define INC_WLAN_P2P_H
...
struct p2p_go_neg_results {
     ...
     */
    uint8_t peer_device_addr[ETH_ALEN];

    /**
     * peer_interface_addr - P2P Interface Address of the peer
     */
    uint8_t peer_interface_addr[ETH_ALEN];
...
struct p2ps_provision {
     ...
     */
    uint8_t cpt_priority[P2PS_FEATURE_CAPAB_CPT_MAX + 1];
...
struct p2p_peer_info {
     ...
     */
    uint8_t wps_sec_dev_type_list[WPS_SEC_DEV_TYPE_MAX_LEN];
...
#define P2P_SEC_DEVICE_TYPES 5
     ...
     */
    uint8_t sec_dev_type[P2P_SEC_DEVICE_TYPES][8];
     ...
     */
    uint8_t ssid_postfix[SSID_MAX_LEN - 9];
    ...
    int (*p2p_scan)(void *ctx, enum p2p_scan_type type, int freq,
            ...
            const uint8_t *req_dev_types, const uint8_t *dev_id, uint16_t pw_id,
            bool include_6ghz);
     ...
     * transmit.
     */
    int (*send_action)(void *ctx, unsigned int freq, const uint8_t *dst,
               const uint8_t *src, const uint8_t *bssid, const uint8_t *buf,
     ...
     */
    int (*get_noa)(void *ctx, const uint8_t *interface_addr, uint8_t *buf,
               size_t buf_len);
     ...
     */
    void (*dev_found)(void *ctx, const uint8_t *addr,
     ...
     */
    void (*dev_lost)(void *ctx, const uint8_t *dev_addr);
     ...
     * call to p2p_connect().
     */
    void (*go_neg_req_rx)(void *ctx, const uint8_t *src, uint16_t dev_passwd_id,
                  uint8_t go_intent);
     ...
     */
    void (*sd_request)(void *ctx, int freq, const uint8_t *sa, uint8_t dialog_token,
     ...
     */
    void (*sd_response)(void *ctx, const uint8_t *sa, uint16_t update_indic,
     ...
     */
    void (*prov_disc_req)(void *ctx, const uint8_t *peer, uint16_t config_methods,
                  const uint8_t *dev_addr, const uint8_t *pri_dev_type,
                  const char *dev_name, uint16_t supp_config_methods,
                  uint8_t dev_capab, uint8_t group_capab,
     ...
     */
    void (*prov_disc_resp)(void *ctx, const uint8_t *peer, uint16_t config_methods);
     ...
     */
    void (*prov_disc_fail)(void *ctx, const uint8_t *peer,
                   enum p2p_prov_disc_status status,
                   uint32_t adv_id, const uint8_t *adv_mac,
     ...
     */
    uint8_t (*invitation_process)(void *ctx, const uint8_t *sa, const uint8_t *bssid,
                 const uint8_t *go_dev_addr, const uint8_t *ssid,
                 size_t ssid_len, int *go, uint8_t *group_bssid,
     ...
     */
    void (*invitation_received)(void *ctx, const uint8_t *sa, const uint8_t *bssid,
                    const uint8_t *ssid, size_t ssid_len,
                    const uint8_t *go_dev_addr, uint8_t status,
                    int op_freq);
     ...
     */
    void (*invitation_result)(void *ctx, int status, const uint8_t *bssid,
                  const struct p2p_channels *channels,
                  const uint8_t *addr, int freq, int peer_oper_freq);
     ...
     * or 0 if not.
     */
    int (*go_connected)(void *ctx, const uint8_t *dev_addr);
     ...
     */
    void (*presence_resp)(void *ctx, const uint8_t *src, uint8_t status,
                  const uint8_t *noa, size_t noa_len);
     ...
     */
    int (*get_persistent_group)(void *ctx, const uint8_t *addr, const uint8_t *ssid,
                    size_t ssid_len, uint8_t *go_dev_addr,
                    uint8_t *ret_ssid, size_t *ret_ssid_len,
                    uint8_t *intended_iface_addr);
     ...
     */
    int (*get_go_info)(void *ctx, uint8_t *intended_addr,
               uint8_t *ssid, size_t *ssid_len, int *group_iface,
               unsigned int *freq);
     ...
     */
    int (*remove_stale_groups)(void *ctx, const uint8_t *peer, const uint8_t *go,
                   const uint8_t *ssid, size_t ssid_len);
     ...
     */
    void (*p2ps_prov_complete)(void *ctx, uint8_t status, const uint8_t *dev,
                   const uint8_t *adv_mac, const uint8_t *ses_mac,
                   const uint8_t *grp_mac, uint32_t adv_id, uint32_t ses_id,
                   uint8_t conncap, int passwd_id,
                   const uint8_t *persist_ssid,
                   size_t persist_ssid_size, int response_done,
                   int prov_start, const char *session_info,
                   const uint8_t *feat_cap, size_t feat_cap_len,
                   unsigned int freq, const uint8_t *group_ssid,
     ...
     */
    uint8_t (*p2ps_group_capability)(void *ctx, uint8_t incoming, uint8_t role,
                    unsigned int *force_freq,
                    unsigned int *pref_freq);
 ...
 * GO Negotiation.
 */
int p2p_unauthorize(struct p2p_data *p2p, const uint8_t *addr);
...
void p2p_set_config_methods(struct p2p_data *p2p, uint16_t config_methods);
void p2p_set_uuid(struct p2p_data *p2p, const uint8_t *uuid);
 ...
 * information that was not available at the time of the p2p_init() call.
 */
int p2p_set_pri_dev_type(struct p2p_data *p2p, const uint8_t *pri_dev_type);
 ...
 * information that was not available at the time of the p2p_init() call.
 */
int p2p_set_sec_dev_types(struct p2p_data *p2p, const uint8_t dev_types[][8],
...
int p2p_find(struct p2p_data *p2p, unsigned int timeout,
         ...
         uint8_t seek_count, const char **seek_string, int freq,
         bool include_6ghz);
 ...
 * Returns: 0 on success, -1 on failure
 */
int p2p_connect(struct p2p_data *p2p, const uint8_t *peer_addr,
        enum p2p_wps_method wps_method,
        int go_intent, const uint8_t *own_interface_addr,
        unsigned int force_freq, int persistent_group,
        const uint8_t *force_ssid, size_t force_ssid_len,
        int pd_before_go_neg, unsigned int pref_freq, uint16_t oob_pw_id);
 ...
 */
int p2p_authorize(struct p2p_data *p2p, const uint8_t *peer_addr,
          enum p2p_wps_method wps_method,
          int go_intent, const uint8_t *own_interface_addr,
          unsigned int force_freq, int persistent_group,
          const uint8_t *force_ssid, size_t force_ssid_len,
          unsigned int pref_freq, uint16_t oob_pw_id);
 ...
 * Returns: 0 on success, -1 on failure
 */
int p2p_reject(struct p2p_data *p2p, const uint8_t *peer_addr);
 ...
 */
int p2p_prov_disc_req(struct p2p_data *p2p, const uint8_t *peer_addr,
              struct p2ps_provision *p2ps_prov, uint16_t config_methods,
 ...
 * callback.
 */
void * p2p_sd_request(struct p2p_data *p2p, const uint8_t *dst,
              const struct wpabuf *tlvs);

#ifdef CONFIG_WIFI_DISPLAY
void * p2p_sd_request_wfd(struct p2p_data *p2p, const uint8_t *dst,
              const struct wpabuf *tlvs);
 ...
 */
void p2p_sd_response(struct p2p_data *p2p, int freq, const uint8_t *dst,
             uint8_t dialog_token, const struct wpabuf *resp_tlvs);
 ...
 * Returns: 0 on success, -1 on failure
 */
int p2p_invite(struct p2p_data *p2p, const uint8_t *peer, enum p2p_invite_role role,
           const uint8_t *bssid, const uint8_t *ssid, size_t ssid_len,
           unsigned int force_freq, const uint8_t *go_dev_addr,
 ...
 */
int p2p_presence_req(struct p2p_data *p2p, const uint8_t *go_interface_addr,
             const uint8_t *own_interface_addr, unsigned int freq,
             uint32_t duration1, uint32_t interval1, uint32_t duration2,
             uint32_t interval2);
 ...
 */
void p2p_wps_success_cb(struct p2p_data *p2p, const uint8_t *mac_addr);
 ...
 * peer.
 */
uint16_t p2p_get_provisioning_info(struct p2p_data *p2p, const uint8_t *addr);
 ...
 * peer.
 */
void p2p_clear_provisioning_info(struct p2p_data *p2p, const uint8_t *addr);
 ...
 */
enum p2p_probe_req_status
p2p_probe_req_rx(struct p2p_data *p2p, const uint8_t *addr, const uint8_t *dst,
 ...
 */
void p2p_rx_action(struct p2p_data *p2p, const uint8_t *da, const uint8_t *sa,
           const uint8_t *bssid, uint8_t category,
           const uint8_t *data, size_t len, int freq);
 ...
 */
int p2p_scan_res_handler(struct p2p_data *p2p, const uint8_t *bssid, int freq,
             struct os_reltime *rx_time, int level, const uint8_t *ies,
             size_t ies_len);
 ...
 */
void p2p_send_action_cb(struct p2p_data *p2p, unsigned int freq, const uint8_t *dst,
...
void p2p_deauth_notif(struct p2p_data *p2p, const uint8_t *bssid, uint16_t reason_code,
              const uint8_t *ie, size_t ie_len);

void p2p_disassoc_notif(struct p2p_data *p2p, const uint8_t *bssid, uint16_t reason_code,
            const uint8_t *ie, size_t ie_len);
 ...
 * Returns: 0 on success, -1 on failure
 */
int p2p_group_notif_assoc(struct p2p_group *group, const uint8_t *addr,
 ...
 * wpabuf_free().
 */
struct wpabuf * p2p_group_assoc_resp_ie(struct p2p_group *group, uint8_t status);
 ...
 * @addr: Interface address of the P2P client
 */
void p2p_group_notif_disassoc(struct p2p_group *group, const uint8_t *addr);
 ...
 */
int p2p_group_notif_noa(struct p2p_group *group, const uint8_t *noa,
            size_t noa_len);
 ...
 */
int p2p_group_go_discover(struct p2p_group *group, const uint8_t *dev_id,
              const uint8_t *searching_dev, int rx_freq);
 ...
 * field=value lines.
 */
int p2p_scan_result_text(const uint8_t *ies, size_t ies_len, char *buf, char *end);
 ...
 */
int p2p_parse_dev_addr_in_p2p_ie(struct wpabuf *p2p_ie, uint8_t *dev_addr);
 ...
 * Returns: 0 on success or -1 if P2P Device Address could not be parsed
 */
int p2p_parse_dev_addr(const uint8_t *ies, size_t ies_len, uint8_t *dev_addr);
 ...
 */
int p2p_assoc_req_ie(struct p2p_data *p2p, const uint8_t *bssid, uint8_t *buf,
 ...
 */
void p2p_scan_ie(struct p2p_data *p2p, struct wpabuf *ies, const uint8_t *dev_id,
         unsigned int bands);
 ...
 */
uint8_t p2p_get_group_capab(const struct wpabuf *p2p_ie);
 ...
 */
const uint8_t * p2p_get_go_dev_addr(const struct wpabuf *p2p_ie);
 ...
 */
int p2p_peer_known(struct p2p_data *p2p, const uint8_t *addr);
 ...
 * returned on success.
 */
int p2p_config_get_random_social(struct p2p_config *p2p, uint8_t *op_class,
                 uint8_t *op_channel,
                 struct wpa_freq_range_list *avoid_list,
                 struct wpa_freq_range_list *disallow_list);

int p2p_set_listen_channel(struct p2p_data *p2p, uint8_t reg_class, uint8_t channel,
               uint8_t forced);

uint8_t p2p_get_listen_channel(struct p2p_data *p2p);

int p2p_set_ssid_postfix(struct p2p_data *p2p, const uint8_t *postfix, size_t len);

int p2p_get_interface_addr(struct p2p_data *p2p, const uint8_t *dev_addr,
               uint8_t *iface_addr);
int p2p_get_dev_addr(struct p2p_data *p2p, const uint8_t *iface_addr,
               uint8_t *dev_addr);

void p2p_set_peer_filter(struct p2p_data *p2p, const uint8_t *addr);
...
int p2p_get_oper_freq(struct p2p_data *p2p, const uint8_t *iface_addr);
...
const uint8_t * p2p_get_go_neg_peer(struct p2p_data *p2p);
 ...
 */
const uint8_t * p2p_iterate_group_members(struct p2p_group *group, void **next);
 ...
 * found
 */
const uint8_t * p2p_group_get_client_interface_addr(struct p2p_group *group,
 ...
 * found
 */
const uint8_t * p2p_group_get_dev_addr(struct p2p_group *group, const uint8_t *addr);
 ...
 */
int p2p_group_is_client_connected(struct p2p_group *group, const uint8_t *dev_addr);
...
p2p_get_peer_found(struct p2p_data *p2p, const uint8_t *addr, int next);
 ...
 * Returns: 0 on success, -1 on failure
 */
int p2p_set_oper_channel(struct p2p_data *p2p, uint8_t op_reg_class, uint8_t op_channel,
             int cfg_op_channel);
 ...
 */
void p2p_set_config_timeout(struct p2p_data *p2p, uint8_t go_timeout,
...
struct p2p_nfc_params {
    int sel;
    const uint8_t *wsc_attr;
    size_t wsc_len;
    const uint8_t *p2p_attr;
    size_t p2p_len;
    ...
    uint8_t oob_dev_pw[WPS_OOB_PUBKEY_HASH_LEN + 2 +
              WPS_OOB_DEVICE_PASSWORD_LEN];
    size_t oob_dev_pw_len;
    int go_freq;
    uint8_t go_dev_addr[ETH_ALEN];
    uint8_t go_ssid[SSID_MAX_LEN];
    size_t go_ssid_len;
};
...
void p2p_set_authorized_oob_dev_pw_id(struct p2p_data *p2p, uint16_t dev_pw_id,
                      int go_intent,
                      const uint8_t *own_interface_addr);
...
void p2p_set_intended_addr(struct p2p_data *p2p, const uint8_t *intended_addr);

struct p2ps_advertisement *
p2p_service_p2ps_id(struct p2p_data *p2p, uint32_t adv_id);
int p2p_service_add_asp(struct p2p_data *p2p, int auto_accept, uint32_t adv_id,
            const char *adv_str, uint8_t svc_state,
            uint16_t config_methods, const char *svc_info,
            const uint8_t *cpt_priority);
int p2p_service_del_asp(struct p2p_data *p2p, uint32_t adv_id);
...
void p2p_set_own_pref_freq_list(struct p2p_data *p2p,
                ...
                unsigned int size);
void p2p_set_override_pref_op_chan(struct p2p_data *p2p, uint8_t op_class,
                   uint8_t chan);
...
bool p2p_is_peer_6ghz_capab(struct p2p_data *p2p, const uint8_t *addr);
bool p2p_peer_wfd_enabled(struct p2p_data *p2p, const uint8_t *peer_addr);
