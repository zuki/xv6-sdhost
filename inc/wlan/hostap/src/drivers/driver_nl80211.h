...
#ifndef INC_WLAN_DRIVERS_DRIVER_NL80211_H
#define INC_WLAN_DRIVERS_DRIVER_NL80211_H
...
struct wpa_driver_nl80211_data {
	...
	struct drv_nl80211_iface_capa {
		...
		uint8_t *ext_capa, *ext_capa_mask;
	...
	uint8_t auth_bssid[ETH_ALEN];
	uint8_t auth_attempt_bssid[ETH_ALEN];
	...
	uint32_t ignore_next_local_disconnect;
	uint32_t ignore_next_local_deauth;

	uint64_t vendor_scan_cookie;
	uint64_t remain_on_chan_cookie;
	uint64_t send_frame_cookie;
	int send_frame_link_id;
#define MAX_SEND_FRAME_COOKIES 20
	uint64_t send_frame_cookies[MAX_SEND_FRAME_COOKIES];
	unsigned int num_send_frame_cookies;
	uint64_t eapol_tx_cookie;
	...
	int auth_freq;
	uint8_t auth_bssid_[ETH_ALEN];
	uint8_t auth_ssid[SSID_MAX_LEN];
	...
	int auth_p2p;
	bool auth_mld;
	uint8_t auth_mld_link_id;
	uint8_t auth_ap_mld_addr[ETH_ALEN];
	...
	uint8_t *pending_roam_data;
	size_t pending_roam_data_len;
	uint8_t *pending_t2lm_data;
	size_t pending_t2lm_data_len;
	uint8_t *pending_link_reconfig_data;
...
int nl80211_create_iface(struct wpa_driver_nl80211_data *drv,
			 ...
			 const uint8_t *addr, int wds,
			 int (*handler)(struct nl_msg *, void *),
			 void *arg, int use_existing);
void nl80211_remove_iface(struct wpa_driver_nl80211_data *drv, int ifidx);
unsigned int nl80211_get_assoc_freq(struct wpa_driver_nl80211_data *drv);
int nl80211_get_assoc_ssid(struct wpa_driver_nl80211_data *drv, uint8_t *ssid);
...
int wpa_driver_nl80211_mlme(struct wpa_driver_nl80211_data *drv,
			    const uint8_t *addr, int cmd, uint16_t reason_code,
...
nl80211_get_hw_feature_data(void *priv, uint16_t *num_modes, uint16_t *flags,
			    uint8_t *dfs_domain);
...
struct i802_link * nl80211_get_link(struct i802_bss *bss, int8_t link_id);

static inline bool nl80211_link_valid(uint16_t links, int8_t link_id)
{
...
int wpa_driver_nl80211_abort_scan(void *priv, uint64_t scan_cookie);
int wpa_driver_nl80211_vendor_scan(struct i802_bss *bss,
				   struct wpa_driver_scan_params *params);
int nl80211_set_default_scan_ies(void *priv, const uint8_t *ies, size_t ies_len);
