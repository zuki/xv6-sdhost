...
#ifndef INC_WLAN_DRIVERS_DRIVER_H
#define INC_WLAN_DRIVERS_DRIVER_H
...
struct eht_capabilities {
    bool eht_supported;
    uint16_t mac_cap;
    uint8_t phy_cap[EHT_PHY_CAPAB_LEN];
    uint8_t mcs[EHT_MCS_NSS_CAPAB_LEN];
    uint8_t ppet[EHT_PPE_THRESH_CAPAB_LEN];
};
...
struct unsol_bcast_probe_resp {
     ...
     */
    uint8_t *unsol_bcast_probe_resp_tmpl;
...
struct wpa_driver_set_key_params {
    /**
     * ifname - インタフェース名 (マルチ-SSID/VLAN サポート用) */
    const char *ifname;
     ...
     * addr - peer STAのアドレス
     *
     * (ステーションモードでペアワイズキーを設定する際の、現在のAPのBSSID),
     * ff:ff:ff:ff:ff:ff はブロードキャストキー、%NULL はブロードキャストと
     * ユニキャストの両方で使用されるデフォルトキーを表す。キーをクリアする際、
     * %NULL を使用すると、指定されたキーインデックスの「ブロードキャスト専用キー」と
     * 「デフォルトキー」の両方がクリアされることを示す */
    const uint8_t *addr;

    /**
     * key_idx - キーインデックス
     *
     * (0..3), ユニキャストキーは通常 0を使う; 4..5 は IGTK 用; 6..7 は BIGTK 用 */
    int key_idx;

    /**
     * set_tx - このキーをデフォルトTxキーとして構成する
     *
     * ドライバーがユニキャストと個別キーを個別にサポートしていない場合にのみ使用される */
    int set_tx;

    /**
     * seq - シーケンス番号/パケット番号
     *
     * sseq_len オクテット。リプレイ保護のために使用される次のパケット番号。Rxキー用に
     * 構成される（ほとんどの場合、これはブロードキャストキーでのみ使用され、ユニキャスト
     * キーではゼロをセットする）。セットしない場合は %NULL  */
    const uint8_t *seq;

    /**
     * seq_len - seqの長さで、アルゴリズムによる
     *
     ...
     * key - キーバッファ
     *
     * TKIP: 16-バイトの一時キーで、8-バイトは Tx Mic キー, 8-バイトは Rx Mic キー */
    const uint8_t *key;

    /**
     * key_len - キーバッファのオクテット単位の長さ
     *
     * WEP: 5 or 13, TKIP: 32, CCMP/GCMP: 16, IGTK: 16 */
    size_t key_len;

    /**
     * vlan_id - VLAN オフロード用のVLANインデックス (0..4095) */
    int vlan_id;
     ...
     * link_id - MLO Link ID
     *
     * 適用可能な場合、正しいLink ID (0-14)をセットする、それ以外は -1. */
    int link_id;
};
...
struct hostap_sta_driver_data {
    ...
    uint8_t rx_vht_nss;
    uint8_t tx_vht_nss;
    int8_t avg_signal; /* dBm */
    int8_t avg_beacon_signal; /* dBm */
    int8_t avg_ack_signal; /* dBm */
    enum guard_interval rx_guard_interval, tx_guard_interval;
    uint8_t rx_dcm, tx_dcm;
};
...
struct cca_settings {
    ...
    uint16_t counter_offset_beacon;
    uint16_t counter_offset_presp;
...
struct weighted_pcl {
    uint32_t freq; /* MHz */
    uint8_t weight;
    uint32_t flag; /* bitmap for WEIGHTED_PCL_* */
};

struct driver_sta_mlo_info {
    bool default_map;
    uint16_t req_links; /* bitmap of requested link IDs */
    uint16_t valid_links; /* bitmap of accepted link IDs */
    uint8_t assoc_link_id;
...
struct wpa_driver_ops {
     ...
     * @priv: プライベートドライバインタフェースデータ from init()
     *
    ...
    /**
     * get_scan_results2 - 最新のスキャン結果を取得する
     * @priv: プライベートドライバインタフェースデータ
     *
     * Returns: 成功時はスキャン結果を保持する割り当てたバッファ
     * （呼び出し元はこのデータ構造体を解放する必要がある）、失敗時は NULL
     */
    ...
    /**
     * scan2 - スキャンを開始するようドライバに要求する
     * @priv: プライベートドライバインタフェースデータ
     * @params: スキャンパラメタ
     *
     * Returns: 0 on success, -1 on failure
     *
     * スキャン結果の準備できたらドライバは wpa_supplicant に対して
     * スキャン結果イベントを通知する必要がある。これにより wpa_supplicant は
     * wpa_driver_get_scan_results2() を使用して結果を要求することになる。
     */
     ...
     */
    int (*tx_control_port)(void *priv, const uint8_t *dest,
    ...
    int (*hapd_send_eapol)(void *priv, const uint8_t *addr, const uint8_t *data,
                   ...
                   const uint8_t *own_addr, uint32_t flags, int link_id);
     ...
     * Returns: 0 on success, -1 on failure (or if not supported)
     */
    int (*set_replay_protect)(void *priv, bool enabled, uint32_t window);
     ...
     * Returns: 0 on success, -1 on failure (or if not supported)
     */
    int (*set_offload)(void *priv, uint8_t offload);
     ...
     * Returns 0 on success, -1 on failure
     */
    int (*set_band)(void *priv, uint32_t band_mask);
    ...
    int (*get_mld_capab)(void *priv, enum wpa_driver_if_type type,
                 uint16_t *eml_capa, uint16_t *mld_capa_and_ops);
     ...
     */
    int (*link_add)(void *priv, uint8_t link_id, const uint8_t *addr, void *bss_ctx);
    ...
    int (*link_remove)(void *priv, enum wpa_driver_if_type type,
               const char *ifname, uint8_t link_id);
     ...
     * Returns: 0 on success, negative value on failure
     */
    int (*link_sta_remove)(void *priv, uint8_t link_id, const uint8_t *addr);

#ifdef CONFIG_TESTING_OPTIONS
    int (*register_frame)(void *priv, uint16_t type,
...
static inline void drv_event_assoc(void *ctx, const uint8_t *addr, const uint8_t *req_ies,
                   size_t req_ielen, const uint8_t *resp_ies,
                   size_t resp_ielen, const uint8_t *link_addr,
...
static inline void drv_event_eapol_rx2(void *ctx, const uint8_t *src, const uint8_t *data,
                       size_t data_len,
                       enum frame_encryption encrypted,
                       int link_id)
{
...
const char * driver_flag2_to_string(uint64_t flag2);
...
#ifdef CONFIG_DRIVER_XV6
extern const struct wpa_driver_ops wpa_driver_xv6_ops; /* driver_xv6.c */
#endif /* CONFIG_DRIVER_XV6 */
