...
#ifndef INC_WLAN_COMMON_IEEE802_11_DEFS_H
#define INC_WLAN_COMMON_IEEE802_11_DEFS_H
...
struct ieee80211_hdr_s1g_beacon {
	...
	/* followed by:
	 *   'uint8_t next_tbtt[3];' if the Next TBTT Present field in the
	 *			Frame Control field is 1
	 *   'uint8_t compressed_ssid[4];' if the Compressed SSID Present field in
	 *			the Frame Control is 1
	 *   'uint8_t ano[1];' if the ANO field in the Frame Control field is 1
	 */
} STRUCT_PACKED;
...
#define VHT_OPMODE_CHANNEL_40MHZ		    ((uint8_t) BIT(0))
#define VHT_OPMODE_CHANNEL_80MHZ		    ((uint8_t) BIT(1))
#define VHT_OPMODE_CHANNEL_160MHZ		    ((uint8_t) BIT(1) | BIT(2))
...
struct wnm_event_report_element {
	uint8_t eid; /* WLAN_EID_EVENT_REPORT */
	uint8_t len;
	uint8_t token;
	uint8_t type;
	uint8_t status;
	...
	union {
		struct {
			uint8_t tsf[8]; /* Event TSF */
			uint8_t color_bitmap[8]; /* Event Report field */
		} STRUCT_PACKED bss_color_collision;
		struct {
			uint8_t tsf[8]; /* Event TSF */
			uint8_t color; /* Event Report field */
...
struct ieee80211_eht_oper_info {
	uint8_t control; /* B0..B2: Channel Width */
	uint8_t ccfs0;
	uint8_t ccfs1;
...
struct ieee80211_eht_operation {
	uint8_t oper_params; /* EHT Operation Parameters: EHT_OPER_* bits */
	uint8_t basic_eht_mcs_nss_set[4];
...
#define EHT_PHYCAP_320MHZ_IN_6GHZ_SUPPORT_MASK	((uint8_t) BIT(1))

#define EHT_PHYCAP_SU_BEAMFORMER_IDX		0
#define EHT_PHYCAP_SU_BEAMFORMER		((uint8_t) BIT(5))
#define EHT_PHYCAP_SU_BEAMFORMEE_IDX		0
#define EHT_PHYCAP_SU_BEAMFORMEE		((uint8_t) BIT(6))

#define EHT_PHYCAP_PPE_THRESHOLD_PRESENT_IDX	5
#define EHT_PHYCAP_PPE_THRESHOLD_PRESENT	((uint8_t) BIT(3))

#define EHT_PHYCAP_MU_BEAMFORMER_IDX		7
#define EHT_PHYCAP_MU_BEAMFORMER_80MHZ		((uint8_t) BIT(4))
#define EHT_PHYCAP_MU_BEAMFORMER_160MHZ		((uint8_t) BIT(5))
#define EHT_PHYCAP_MU_BEAMFORMER_320MHZ		((uint8_t) BIT(6))
...
#define EHT_PPE_THRES_NSS_MASK			((uint8_t) (BIT(0) | BIT(1) | \
						       BIT(2) | BIT(3)))
#define EHT_PPE_THRES_RU_INDEX_SHIFT		4
#define EHT_PPE_THRES_RU_INDEX_MASK		((uint16_t) (BIT(4) | BIT(5) | \
							BIT(6) | BIT(7) | \
							BIT(8)))
...
struct ieee80211_eht_capabilities {
	...
	uint8_t optional[EHT_MCS_NSS_CAPAB_LEN + EHT_PPE_THRESH_CAPAB_LEN];
} STRUCT_PACKED;
 ...
 */
#define SCS_QOS_BIT_MAX_MSDU_SIZE			((uint16_t) BIT(0))
#define SCS_QOS_BIT_SERVICE_START_TIME			((uint16_t) BIT(1))
#define SCS_QOS_BIT_SERVICE_START_TIME_LINKID		((uint16_t) BIT(2))
#define SCS_QOS_BIT_MEAN_DATA_RATE			((uint16_t) BIT(3))
#define SCS_QOS_BIT_DELAYED_BOUNDED_BURST_SIZE		((uint16_t) BIT(4))
#define SCS_QOS_BIT_MSDU_LIFETIME			((uint16_t) BIT(5))
#define SCS_QOS_BIT_MSDU_DELIVERY_INFO			((uint16_t) BIT(6))
#define SCS_QOS_BIT_MEDIUM_TIME				((uint16_t) BIT(7))
 ...
 */
#define FD_FRAME_CTL_CAP_PRESENT			((uint16_t) BIT(5))
#define FD_FRAME_CTL_SHORT_SSID_PRESENT			((uint16_t) BIT(6))
#define FD_FRAME_CTL_AP_CSN_PRESENT			((uint16_t) BIT(7))
#define FD_FRAME_CTL_ANO_PRESENT			((uint16_t) BIT(8))
#define FD_FRAME_CTL_FREQ_SEG1_PRESENT			((uint16_t) BIT(9))
#define FD_FRAME_CTL_PRI_CHAN_PRESENT			((uint16_t) BIT(10))
#define FD_FRAME_CTL_RSN_INFO_PRESENT			((uint16_t) BIT(11))
#define FD_FRAME_CTL_LENGTH_PRESENT			((uint16_t) BIT(12))
#define FD_FRAME_CTL_MD_PRESENT				((uint16_t) BIT(13))
