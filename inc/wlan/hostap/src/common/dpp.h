...
#ifndef INC_WLAN_COMMON_DPP_H
#define INC_WLAN_COMMON_DPP_H
...
struct dpp_bootstrap_info {
	...
	uint8_t version;
	uint8_t supported_curves; /* enum dpp_bootstrap_supported_curves bitmap */
	int own;
	struct crypto_ec_key *pubkey;
	uint8_t pubkey_hash[SHA256_MAC_LEN];
	uint8_t pubkey_hash_chirp[SHA256_MAC_LEN];
	...
	uint8_t peer_pubkey_hash[SHA256_MAC_LEN]; /* for enforcing a specific
...
struct dpp_authentication {
	...
	uint8_t c_nonce[DPP_MAX_NONCE_LEN];
	uint8_t i_capab;
	uint8_t r_capab;
...
struct dpp_authentication * dpp_auth_init(struct dpp_global *dpp, void *msg_ctx,
					  ...
					  uint16_t num_modes);
struct dpp_authentication *
dpp_auth_req_rx(struct dpp_global *dpp, void *msg_ctx, uint8_t dpp_allowed_roles,
...
struct wpabuf *
dpp_build_conf_resp(struct dpp_authentication *auth, const uint8_t *e_nonce,
		    uint16_t e_nonce_len, enum dpp_netrole netrole,
		    bool cert_req);
struct wpabuf *
...
dpp_peer_intro(struct dpp_introduction *intro, const char *own_connector,
	       ...
	       os_time_t *expiry, uint8_t *peer_key_hash);
...
struct dpp_pkex * dpp_pkex_rx_exchange_req(void *msg_ctx,
					   ...
					   const uint8_t *buf, size_t len, bool v2);
...
void dpp_notify_chirp_received(void *msg_ctx, int id, const uint8_t *src,
				unsigned int freq, const uint8_t *hash);
...
struct wpabuf * dpp_build_pb_announcement_resp(struct dpp_bootstrap_info *bi,
					       const uint8_t *e_hash,
					       const uint8_t *c_nonce,
					       size_t c_nonce_len);
...
struct wpabuf * dpp_build_reconfig_announcement(const uint8_t *csign_key,
...
dpp_reconfig_init(struct dpp_global *dpp, void *msg_ctx,
		  struct dpp_configurator *conf, unsigned int freq, uint16_t group,
		  const uint8_t *a_nonce_attr, size_t a_nonce_len,
		  const uint8_t *e_id_attr, size_t e_id_len);
struct dpp_authentication *
dpp_reconfig_auth_req_rx(struct dpp_global *dpp, void *msg_ctx,
			 const char *own_connector,
			 const uint8_t *net_access_key, size_t net_access_key_len,
			 const uint8_t *csign_key, size_t csign_key_len,
			 unsigned int freq, const uint8_t *hdr,
			 const uint8_t *attr_start, size_t attr_len);
struct wpabuf *
dpp_reconfig_auth_resp_rx(struct dpp_authentication *auth, const uint8_t *hdr,
			  const uint8_t *attr_start, size_t attr_len);
int dpp_reconfig_auth_conf_rx(struct dpp_authentication *auth, const uint8_t *hdr,
			      const uint8_t *attr_start, size_t attr_len);

struct dpp_reconfig_id * dpp_gen_reconfig_id(const uint8_t *csign_key,
					     size_t csign_key_len,
					     const uint8_t *pp_key,
					     size_t pp_key_len);
int dpp_update_reconfig_id(struct dpp_reconfig_id *id);
void dpp_free_reconfig_id(struct dpp_reconfig_id *id);
int dpp_get_pubkey_hash(struct crypto_ec_key *key, uint8_t *hash);
