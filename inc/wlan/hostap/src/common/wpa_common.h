...
#ifndef INC_WLAN_COMMON_WPA_COMMON_H
#define INC_WLAN_COMMON_WPA_COMMON_H
...
struct wpa_ptk {
	...
	uint8_t ltf_keyseed[WPA_LTF_KEYSEED_MAX_LEN]; /* LTF Key seed */
	size_t kck_len;
	size_t kek_len;
	size_t tk_len;
	size_t kck2_len;
...
struct wpa_ft_ies {
	...
	const uint8_t *rsnxe;
	size_t rsnxe_len;
	uint16_t valid_mlo_gtks; /* bitmap of valid link GTK subelements */
	const uint8_t *mlo_gtk[MAX_NUM_MLD_LINKS];
	size_t mlo_gtk_len[MAX_NUM_MLD_LINKS];
	uint16_t valid_mlo_igtks; /* bitmap of valid link IGTK subelements */
	const uint8_t *mlo_igtk[MAX_NUM_MLD_LINKS];
	size_t mlo_igtk_len[MAX_NUM_MLD_LINKS];
	uint16_t valid_mlo_bigtks; /* bitmap of valid link BIGTK subelements */
	const uint8_t *mlo_bigtk[MAX_NUM_MLD_LINKS];
...
struct pasn_parameter_ie {
	...
	uint8_t len;
	uint8_t id_ext;
	uint8_t control; /* WPA_PASN_CTRL_* */
	uint8_t wrapped_data_format; /* WPA_PASN_WRAPPED_DATA_* */
} STRUCT_PACKED;

struct wpa_pasn_params_data {
	uint8_t wrapped_data_format;
	uint16_t after;
	uint8_t comeback_len;
	const uint8_t *comeback;
	uint16_t group;
	uint8_t pubkey_len;
...
struct wpa_eapol_ie_parse {
	...
	const uint8_t *wmm;
	size_t wmm_len;
	uint16_t valid_mlo_gtks; /* bitmap of valid link GTK KDEs */
	const uint8_t *mlo_gtk[MAX_NUM_MLD_LINKS];
	size_t mlo_gtk_len[MAX_NUM_MLD_LINKS];
	uint16_t valid_mlo_igtks; /* bitmap of valid link IGTK KDEs */
	const uint8_t *mlo_igtk[MAX_NUM_MLD_LINKS];
	size_t mlo_igtk_len[MAX_NUM_MLD_LINKS];
	uint16_t valid_mlo_bigtks; /* bitmap of valid link BIGTK KDEs */
	const uint8_t *mlo_bigtk[MAX_NUM_MLD_LINKS];
	size_t mlo_bigtk_len[MAX_NUM_MLD_LINKS];
	uint16_t valid_mlo_links; /* bitmap of valid MLO link KDEs */
	const uint8_t *mlo_link[MAX_NUM_MLD_LINKS];
	size_t mlo_link_len[MAX_NUM_MLD_LINKS];
};

int wpa_parse_kde_ies(const uint8_t *buf, size_t len, struct wpa_eapol_ie_parse *ie);
static inline int wpa_supplicant_parse_ies(const uint8_t *buf, size_t len,
...
int wpa_pasn_add_rsne(struct wpabuf *buf, const uint8_t *pmkid,
		      int akmp, int cipher);
