...
#ifndef INC_WLAN_COMMON_DPP_I_H
#define INC_WLAN_COMMON_DPP_I_H
...
struct dpp_global {
	...
	void *relay_msg_ctx;
	void *relay_cb_ctx;
	void (*relay_tx)(void *ctx, const uint8_t *addr, unsigned int freq,
			 const uint8_t *msg, size_t len);
	void (*relay_gas_resp_tx)(void *ctx, const uint8_t *addr, uint8_t dialog_token,
...
void dpp_build_attr_r_bootstrap_key_hash(struct wpabuf *msg, const uint8_t *hash);
...
int dpp_prepare_channel_list(struct dpp_authentication *auth,
			     ...
			     struct hostapd_hw_modes *own_modes, uint16_t num_modes);
...
int dpp_conf_resp_env_data(struct dpp_authentication *auth,
			   const uint8_t *env_data, size_t env_data_len);
...
const struct dpp_curve_params * dpp_get_curve_ike_group(uint16_t group);
...
int dpp_hkdf_expand(size_t hash_len, const uint8_t *secret, size_t secret_len,
		    const char *label, uint8_t *out, size_t outlen);
int dpp_hmac_vector(size_t hash_len, const uint8_t *key, size_t key_len,
...
int dpp_pbkdf2(size_t hash_len, const uint8_t *password, size_t password_len,
...
int dpp_gen_r_auth(struct dpp_authentication *auth, uint8_t *r_auth);
int dpp_gen_i_auth(struct dpp_authentication *auth, uint8_t *i_auth);
...
int dpp_derive_pmkid(const struct dpp_curve_params *curve,
		     ...
		     struct crypto_ec_key *peer_key, uint8_t *pmkid);
struct crypto_ec_point *
dpp_pkex_derive_Qi(const struct dpp_curve_params *curve, const uint8_t *mac_init,
		   const char *code, size_t code_len, const char *identifier,
		   struct crypto_ec **ret_ec);
struct crypto_ec_point *
dpp_pkex_derive_Qr(const struct dpp_curve_params *curve, const uint8_t *mac_resp,
		   const char *code, size_t code_len, const char *identifier,
		   struct crypto_ec **ret_ec);
int dpp_pkex_derive_z(const uint8_t *mac_init, const uint8_t *mac_resp,
		      uint8_t ver_init, uint8_t ver_resp,
		      const uint8_t *Mx, size_t Mx_len,
		      const uint8_t *Nx, size_t Nx_len,
		      const char *code, size_t code_len,
		      const uint8_t *Kx, size_t Kx_len,
		      uint8_t *z, unsigned int hash_len);
int dpp_reconfig_derive_ke_responder(struct dpp_authentication *auth,
				     const uint8_t *net_access_key,
				     size_t net_access_key_len,
				     struct json_token *peer_net_access_key);
int dpp_reconfig_derive_ke_initiator(struct dpp_authentication *auth,
				     const uint8_t *r_proto, uint16_t r_proto_len,
...
int dpp_derive_auth_i(struct dpp_authentication *auth, uint8_t *auth_i);
