...
#ifndef INC_WLAN_UTILS_CONST_TIME_H
#define INC_WLAN_UTILS_CONST_TIME_H
...
/* Returns: -1 if a == b; 0 if a != b */
static inline uint8_t const_time_eq_uint8_t(unsigned int a, unsigned int b)
{
 ...
 * Returns: true_val if mask == -1, false_val if mask == 0
 */
static inline uint8_t const_time_select_uint8_t(uint8_t mask, uint8_t true_val, uint8_t false_val)
{
					 ...
					 uint8_t *dst)
{
	...
	for (i = 0; i < len; i++)
		dst[i] = const_time_select_uint8_t(mask, true_val[i], false_val[i]);
}
