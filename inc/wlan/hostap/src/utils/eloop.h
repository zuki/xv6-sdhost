...
/**
 * eloop_sock_handler - eloopソケットイベントコールバック型
 * @sock: ソケットのファイルディスクリプタ番号
 * @eloop_ctx: 登録済みのコールバックコンテキストデータ (eloop_data)
 * @sock_ctx: 登録済みのコールバックコンテキストデータ (user_data)
 */
typedef void (*eloop_sock_handler)(int sock, void *eloop_ctx, void *sock_ctx);

/**
 * eloop_event_handler - eloop汎用イベントコールバック型
...
/**
 * eloop_signal_handler - eloopシグナルイベントコールバック型
 * @sig: Signal number
...
/**
 * eloop_init() - グローバルなイベントループデータを初期化する
 * Returns: 0 on success, -1 on failure
 *
 * このカンスはその他のすべてのeloop_*関数より前に呼び出さなければならない.
 */
int eloop_init(void);

/**
 * eloop_register_read_sock - readイベント用のハンドラを登録する
