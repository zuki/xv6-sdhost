...
#ifndef INC_WLAN_FST_FST_CTRL_IFACE_H
#define INC_WLAN_FST_FST_CTRL_IFACE_H
...
/* receiver */
int fst_ctrl_iface_mb_info(const uint8_t *addr, char *buf, size_t buflen);
...
static inline int
fst_ctrl_iface_mb_info(const uint8_t *addr, char *buf, size_t buflen)
{
	return 0;
}
...
int fst_read_next_text_param(const char *params, char *buf, size_t buflen,
			     char **endp);
int fst_read_peer_addr(const char *mac, uint8_t *peer_addr);
