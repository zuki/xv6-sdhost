...
#ifndef INC_WLAN_RSN_SUPP_WPA_I_H
#define INC_WLAN_RSN_SUPP_WPA_I_H
...
/**
 * struct wpa_sm - WPAステートマシンの内部データ
 */
struct wpa_sm {
    ...
    uint8_t *assoc_rsnxe; /* Own RSNXE from (Re)AssocReq */
    size_t assoc_rsnxe_len;
    uint8_t *ap_wpa_ie, *ap_rsn_ie, *ap_rsnxe;
...
static inline int wpa_sm_set_key(struct wpa_sm *sm, int link_id,
                 enum wpa_alg alg, const uint8_t *addr, int key_idx,
                 int set_tx, const uint8_t *seq, size_t seq_len,
...
static inline int wpa_sm_add_pmkid(struct wpa_sm *sm, void *network_ctx,
                   ...
                   size_t pmk_len, uint32_t pmk_lifetime,
...
static inline void wpa_sm_transition_disable(struct wpa_sm *sm, uint8_t bitmap)
{
...
static inline void wpa_sm_store_ptk(struct wpa_sm *sm,
                    const uint8_t *addr, int cipher,
                    uint32_t life_time, struct wpa_ptk *ptk)
{
    if (sm->ctx->store_ptk)
...
#ifdef CONFIG_PASN
static inline int wpa_sm_set_ltf_keyseed(struct wpa_sm *sm, const uint8_t *own_addr,
